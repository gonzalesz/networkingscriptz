...
def loginjumphost():
    ...
    try:
        ...
        tkMessageBox.showinfo('Error', 'Socket timeout; cannot connect to device.')
        quit()
...
def defaultpage():
    ...
    bifacemain=LabelFrame(window,text=" Interface (required for *Apply commands below) ",font=('verdana', 8, 'bold'),padx=122,pady=10,width=100,height=100)
    ...
    def setinterfaceipclick():
        ...
        if getiface.get() == "" or getiface.get() == "e.g. fa0/0, fa0/0.2, vlan1, tun0, bvi1, po1 ..":
            tkMessageBox.showinfo('Error', 'Please select an interface above.')
        else:
...
##                            Shutstatetxt = "Interface is enabled. "
    ...
    def createbridgeG():
        if createbridge.get() == "" or createbridge.get() == "eg. 5":
            tkMessageBox.showinfo('ERROR', 'Please enter bridge group value.')
        else:
            if chk_state_neg.get() == False: 
                remote_conn.send("conf t \n")
                sleep(0.3)
