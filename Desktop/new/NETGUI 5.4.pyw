...
def login():
    ...
    #ip = "192.168.1.172"        #ip address of router/switch to connect to
    port = 22
    global remote_conn
    ssh_client = paramiko.SSHClient()
    ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
...
##                        pass
                    ...
                    sleep(0.2)
                    remote_conn.send("exit \n") 
                    tkMessageBox.showinfo('IP configuration', IPtxt + Shutstatetxt + tuntxt + bridgetxt)


                else:
                    if (ipaddr.get() == "" or ipaddr.get() == "IP. 'del' to delete.") and (ipmask.get() == "" or ipmask.get() == "Mask. 'del' to delete.") \
                       and (tunsrc.get() == "" or tunsrc.get() == "Src IP. 'del' to delete.") and (tundst.get() == "" or tundst.get() == "Dst IP. 'del' to delete."):
                        ...
                        remote_conn.send("exit \n")
                        sleep(0.2)
                        tkMessageBox.showinfo('IP configuration', 'Interface deleted.')
                    else:
                        remote_conn.send("sh ip int " + getiface.get() + " \n")
                        sleep(0.5)
                        output = remote_conn.recv(2048).decode("utf-8")
                        ...
                        sleep(0.2)
                        if 'Invalid input' in stripp:
                            tkMessageBox.showinfo('Error', 'The interface ' + getiface.get() + ' does not exist... Aborting.')
                            return
                        else:
                            pass
...
##                            pass
                        ...
                        if ipaddr.get() == "del" and ipmask.get() == "del":
                            if chk_state_sec.get() == False:
                                ...
                                IPtxt = "Secondary IP address removed. "
                        elif (ipaddr.get() != "" or ipaddr.get() != "IP. 'del' to delete.") and (ipmask.get() != "" or ipmask.get() != "Mask. 'del' to delete."):
                            tkMessageBox.showinfo('Alert', 'Please use the "del" keyword in IP and mask fields for deletion of IP.')
                            sleep(0.2)   
                            remote_conn.send("exit \n")
                            sleep(0.2)   
                            remote_conn.send("exit \n")
...
##                            Shutstatetxt = "Interface is enabled. "
                        ...
                        sleep(0.2)
                        remote_conn.send("exit \n") 
                        tkMessageBox.showinfo('IP configuration', IPtxt + tuntxt)
    ...
    def setspeedduplex():
        if getiface.get() == "" or getiface.get() == "e.g. fa0/0, fa0/0.2, vlan1, tun0, bvi1, po1 ..":
            ...
            tkMessageBox.showinfo('Error', 'Can only set speed and duplex for physical interfaces.')
        elif ("fa" in getiface.get() or "Fa" in getiface.get()) and spd.get() == "1000":
            tkMessageBox.showinfo('Error', 'Error - Speed for FastEthernet can be configured only up to 100Mbps.')
        else:    
            remote_conn.send("conf t \n")
            sleep(0.5)
            remote_conn.send("int " + getiface.get() + "\n")
            ...
            if chk_state_neg.get() == False:
                ...
                if otheroptn.get() == 'MAC address':
                    ...
                    if 'tu' not in getiface.get():
                        tkMessageBox.showinfo('Error', 'Error - Only can apply fVRF to tunnel interfaces.')
                    elif otherinput.get() == "":
                        tkMessageBox.showinfo('Error', 'Error - Please enter a VRF to apply on the interface.')
                    elif otherinput.get() not in output:
                        tkMessageBox.showinfo('Error', 'Error - Please create the VRF first (in Route page) before applying it.')
                    ...
                    if otherinput.get() == "<bridge-grp no.>" or otherinput.get() == "":
                        tkMessageBox.showinfo('Error', 'Error - Please enter a bridge group number.')
                    else:
                        sleep(0.5)
                        remote_conn.send("int " + getiface.get() + " \n")
                        sleep(0.5)
                    ...
                    if otherinput.get() == "" or otherinput.get() == "<secs> [<retries>]":
                        tkMessageBox.showinfo('Error', 'Error - Please enter a keepalive value to apply on the interface.')
                    elif getiface.get()[:2] != 'tu':
                        findfirstdigits = re.search(r'\d+', otherinput.get()).group()
                        sleep(0.2)
                        remote_conn.send("keepalive " + findfirstdigits + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Other interface settings', 'Interface keepalive configured.')
                    else:
                        remote_conn.send("keepalive " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Other interface settings', 'Tunnel interface keepalive configured.')
                elif otheroptn.get() == 'bandwidth':
                    if otherinput.get() == "" or otherinput.get() == "<kilobits>":
                        tkMessageBox.showinfo('Error', 'Error - Please enter a bandwidth value to apply on the interface.')
                    else:
                        remote_conn.send("bandwidth " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Other interface settings', 'Interface bandwidth configured.')
                ...
                if otheroptn.get() == 'MAC address':
                    ...
                    if getiface.get()[:2] != 'tu':
                        ...
                        remote_conn.send("no bandwidth \n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Other interface settings', 'Interface bandwidth removed.')
                elif otheroptn.get() == 'IPSEC profile':
                    if getiface.get()[:2] != 'tu':
    ...
    deviceconfframe=LabelFrame(window,text=" Device Config and tuning",font=('verdana', 8, 'bold'),padx=10,pady=8,width=100,height=100)
...
if __name__ == "__main__":
        ...
        main()
        window.withdraw()
        jumphostip = askstring("", "Enter Jumphost IP (if none, type 'n'):")
        ip = askstring("", "Enter host IP:")
        if jumphostip == "" or ip == "":
            window.destroy()
