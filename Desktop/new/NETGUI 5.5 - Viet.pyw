...
localip = "172.30.112.214"    #địa chỉ ip của svr Syslog của bạn (cho chức năng 'debugging')
...
def loginjumphost():
    ...
    try:
        ...
        tkMessageBox.showinfo('Lỗi', 'Thời gian chờ kết nối; không thể kết nối với thiết bị.')
        quit()
...
##                            tkMessageBox.showinfo('BGP ' + i, 'This BGP process is already running', parent=window)
                        ...
                        if AFIvar.get() == 1:
                            if "(" not in bgpproc.get():
                                ...
                                remote_conn.send("exit \n")
                                sleep(0.2)
                                tkMessageBox.showinfo('Error', 'BGP enabled, AF is not. Please select either an IPv4 or IPv6 AF to apply VRFs.', parent=window)
                                sleep(0.2)
                                return
                        elif AFIvar.get() == 2:
                            if "(" not in bgpproc.get():
                    ...
                    if bgpproc.get() == "" or bgpproc.get() == "eg. 15(MyVRF)":
                        ...
                        if AFIvar.get() == 1:
                            if "(" not in bgpproc.get():
                                ...
                                remote_conn.send("exit \n")
                                sleep(0.2)
                                tkMessageBox.showinfo('Error', 'Please select either an IPv4 or IPv6 AF to apply VRFs.', parent=window)
                                return
                        elif AFIvar.get() == 2:
                            if "(" not in bgpproc.get():
                                remote_conn.send("address-f ipv4 " + SAFIopt.get() + " \n")
    ...
    def security():
        ...
        def dmvpnclick():
            if dmvpntuniface.get() == "" or dmvpntuniface.get() == "eg. tun0":
                tkMessageBox.showinfo('Error', 'Please enter a tunnel interface to apply DMVPN configuration on.', parent=window)
            else:
                if chk_state_neg.get() == False:
                    res = ""
                    remote_conn.send("conf t \n")
...
def defaultpage():
    ...
    Negateanyframe=LabelFrame(window,text=" Phủ nhận ",font=('verdana', 8, 'bold'),padx=30,pady=7,width=100,height=100)
    ...
    bifacemain=LabelFrame(window,text=" Giao diện (bắt buộc đối với *, tùy chọn đối với **) ",font=('verdana', 8, 'bold'),padx=122,pady=10,width=100,height=100)
    ...
    biface=LabelFrame(window,text=" Cấu hình giao diện ",font=('verdana', 8, 'bold'),padx=17,pady=0,width=100,height=100)
    biface.grid(row=1,column=0, sticky=("nsew"))


    chk_state_sec = BooleanVar()
    chk_state_sec.set(False)
    chk = Checkbutton(biface, text='IP phụ', variable=chk_state_sec)
    chk.grid(column=2, row=2)

    chk_state_nosh = BooleanVar()
    chk_state_nosh.set(True)
    chk1 = Checkbutton(biface, text='Kích hoạt', variable=chk_state_nosh)
    chk1.grid(column=3, row=2)




    lbl = Label(biface, text="Đặt IP và mặt nạ mạng:")
    lbl.grid(column=0, row=3)
    ipaddr = Entry(biface, bg='white', width=17, fg='grey')
    ipaddr.grid(column=1, row=3)
    ipaddr.insert(0, "IP. 'del' để xóa.")
    def handle_focus_in(_):
    ...
    ipmask.insert(0, "Mạng con. 'del' để xóa.")
    def handle_focus_in(_):
    ...
    tundst.insert(0, "Dst IP. 'del' để xóa.")
    def handle_focus_in(_):
    ...
    def setinterfaceipclick():
        ...
        if getiface.get() == "" or getiface.get() == "e.g. fa0/0, fa0/0.2, vlan1, tun0, bvi1, po1 ..":
            tkMessageBox.showinfo('Lỗi', 'Vui lòng chọn một giao diện.')
        else:
            if ((ipaddr.get() != "" and ipaddr.get().encode('utf-8') != "IP. 'del' để xóa.") and (ipmask.get() == "" or ipmask.get().encode('utf-8') == "Mạng con. 'del' để xóa."))\
               or ((ipaddr.get() == "" or ipaddr.get().encode('utf-8') == "IP. 'del' để xóa.") and (ipmask.get() != "" and ipmask.get().encode('utf-8') != "Mạng con. 'del' để xóa.")):
                tkMessageBox.showinfo('Lỗi cấu hình IP', 'Vui lòng nhập cả IP và mặt nạ mạng con để định cấu hình IP.')
            elif chk_state_neg.get() == False and (getiface.get()[:2] == "tu") and ((tunsrc.get() != "" and tunsrc.get().encode('utf-8') != "Src IP. 'del' để xóa.") or\
                 (tundst.get() != "" and tundst.get().encode('utf-8') != "Dst IP. 'del' để xóa.")) and tunmode.get().encode('utf-8') == 'Chế độ':
                tkMessageBox.showinfo('Lỗi', 'Vui lòng chọn chế độ đường hầm trước khi thêm IP đường hầm.')
            elif chk_state_neg.get() == False and (ipaddr.get() == "del" or ipmask.get() == "del" or tunsrc.get() == "del" or tundst.get() == "del"):
                tkMessageBox.showinfo('Lỗi', 'Chọn "Phủ nhận" để chỉ định "del" để xóa địa chỉ IP.')
            else:
                if chk_state_neg.get() == False:
...
##                        pass
                    ...
                    if 'Invalid input' in stripp:
                        sleep(0.2)
                        remote_conn.send("exit \n")
                        tkMessageBox.showinfo('Lỗi', 'Giao diện ' + getiface.get() + ' không tồn tại ... Đang hủy. Nếu BVI, hãy đảm bảo rằng Bridge Group đã được tạo trước.')
                        return
                    else:
                        pass
                    
                    tuntxt = ""
                    if (getiface.get()[:2] == "tu") and ((tunsrc.get() != "" and tunsrc.get().encode('utf-8') != "Src IP. 'del' để xóa.") or\
                                                         (tundst.get() != "" and tundst.get().encode('utf-8') != "Dst IP. 'del' để xóa.")):
                        ...
                        sleep(0.3)
                        tuntxt = "Đã định cấu hình IP đường hầm - đừng quên thêm một tuyến đường để đẩy lưu lượng mạng qua đường hầm này. "                           
                    else:
                        pass
...
##                        if (ipaddr.get() != "" and ipaddr.get().encode('utf-8') != "IP. 'del' để xóa.") and (ipmask.get() != "" and \
##                            ipmask.get().encode('utf-8') != "Mạng con. 'del' để xóa."):
...
##                        pass
                    ...
                    if (ipaddr.get() != "" and ipaddr.get().encode('utf-8') != "IP. 'del' để xóa.") and (ipmask.get() != "" and \
                        ...
                        if chk_state_sec.get() == False:
                            ...
                            sleep(0.3)
                            IPtxt = "IP đã được cấu hình. "
                        else:
                            remote_conn.send(ipaddrcmd + " secondary \n")
                            sleep(0.3)
                            IPtxt = "IP phụ đã được cấu hình. "
                    else:
                        pass
                    ...
                    if chk_state_nosh.get() == False:
                        ...
                        remote_conn.send("sh \n")
                        sleep(0.3)
                        Shutstatetxt = "Giao diện bị tắt. "
                    else:
                        sleep(0.3)
                        remote_conn.send("no sh \n")
                        sleep(0.3)
                        Shutstatetxt = "Giao diện đã được kích hoạt. "

                    sleep(0.2)    
                    remote_conn.send("exit \n")
                    sleep(0.2)
                    remote_conn.send("exit \n") 
                    tkMessageBox.showinfo('Cấu hình IP', IPtxt + Shutstatetxt + tuntxt + bridgetxt)


                else:
                    if (ipaddr.get() == "" or ipaddr.get().encode('utf-8') == "IP. 'del' để xóa.") and (ipmask.get() == "" or ipmask.get().encode('utf-8') == "Mạng con. 'del' để xóa.") \
                       and (tunsrc.get() == "" or tunsrc.get().encode('utf-8') == "Src IP. 'del' để xóa.") and (tundst.get() == "" or tundst.get().encode('utf-8') == "Dst IP. 'del' để xóa."):
                        remote_conn.send("conf t \n")
                        sleep(0.3)
                        ...
                        remote_conn.send("exit \n")
                        sleep(0.2)
                        tkMessageBox.showinfo('Cấu hình IP', 'Interface deleted.')
                    else:
                        remote_conn.send("sh ip int " + getiface.get() + " \n")
                        sleep(0.5)
                        output = remote_conn.recv(2048).decode("utf-8")
                        ...
                        sleep(0.2)
                        if 'Invalid input' in stripp:
                            tkMessageBox.showinfo('Lỗi', 'Giao diện ' + getiface.get() + ' không tồn tại ... Đang hủy.')
                            return
                        else:
                            pass
                        ...
                        if (getiface.get()[:2] == "tu") and ((tunsrc.get() != "" and tunsrc.get().encode('utf-8') != "Src IP. 'del' để xóa.") or\
                            ...
                            if tunsrc.get() == "del":
                                ...
                                sleep(0.3)
                                tuntxt = tuntxt + "Đã xóa IP nguồn đường hầm. "
                            else:
                                pass
                            if tundst.get() == "del":
                                remote_conn.send("no tun dest \n")
                                sleep(0.3)
                                tuntxt = tuntxt + "Đã xóa IP đích của đường hầm. "
                            else:
                                pass
...
##                            pass
                        ...
                        if ipaddr.get() == "del" and ipmask.get() == "del":
                            if chk_state_sec.get() == False:
                                ...
                                IPtxt = "Địa chỉ IP đã bị xóa. "
                            else:
                                remote_conn.send("no ip addr sec \n")
                                IPtxt = "Địa chỉ IP phụ đã bị xóa. "
                        elif (ipaddr.get() != "" or ipaddr.get().encode('utf-8') != "IP. 'del' để xóa.") and (ipmask.get() != "" or ipmask.get().encode('utf-8') != "Mạng con. 'del' để xóa."):
                            tkMessageBox.showinfo('Lỗi', 'Vui lòng sử dụng từ khóa "del" trong các trường IP và mặt nạ để xóa IP.')
                            sleep(0.2)   
                            remote_conn.send("exit \n")
                            sleep(0.2)   
                            remote_conn.send("exit \n")
...
##                            Shutstatetxt = "Interface is enabled. "
                        ...
                        sleep(0.2)
                        remote_conn.send("exit \n") 
                        tkMessageBox.showinfo('Cấu hình IP', IPtxt + tuntxt)
            
    btn = Button(biface, text="*Áp dụng", bg="orange", command=setinterfaceipclick)
    ...
    lbl = Label(biface, text="Tốc độ & song công:").grid(column=0, row=6)
    OPTIONS = [
    "auto",        
    "10",        
    "100",
    ...
    def setspeedduplex():
        if getiface.get() == "" or getiface.get() == "e.g. fa0/0, fa0/0.2, vlan1, tun0, bvi1, po1 ..":
            ...
            tkMessageBox.showinfo('Lỗi', 'Chỉ có thể đặt tốc độ và song công cho các giao diện vật lý.')
        elif ("fa" in getiface.get() or "Fa" in getiface.get()) and spd.get() == "1000":
            tkMessageBox.showinfo('Lỗi', 'Tốc độ cho FastEthernet chỉ có thể được định cấu hình lên đến 100Mbps.')
        else:    
            remote_conn.send("conf t \n")
            sleep(0.5)
            remote_conn.send("int " + getiface.get() + "\n")
            ...
            if 'Invalid input' in stripp:
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('Lỗi', 'Giao diện ' + getiface.get() + ' không tồn tại ... Hủy bỏ. Nếu BVI, hãy đảm bảo Bridge Group đã được tạo trước.')
                return
            else:
                pass
            ...
            sleep(0.5)
            remote_conn.send("exit \n")
            res = "Tốc độ đã được đặt thành " + spd.get() + " và song công đã được đặt thành " + dup.get() + " trên " + getiface.get()
            tkMessageBox.showinfo('Tốc độ & Đảo mặt', res)
    btn = Button(biface, text="*Áp dụng", bg="orange", command=setspeedduplex)
    btn.grid(column=3, row=6)        

    blankdivider = Label(biface, text="").grid(column=0, row=8)
    
    lbl = Label(biface, text="Các thiết lập khác:").grid(column=0, row=9)
    ...
    def option_changed_otheroptn(*args):
        if otheroptn.get() == "MAC address":
            ...
            otherinput.bind("<FocusOut>", handle_focus_out)
            otherinput.bind("<FocusIn>", handle_focus_in)
        elif otheroptn.get().encode('utf-8') == "Thêm vào Bridge":
            text = '<bridge-grp no.>'
            otherinput.delete(0, END)
            otherinput.config(fg='grey')
            otherinput.insert(0, text)
            ...
            if chk_state_neg.get() == False:
                ...
                if otheroptn.get() == 'MAC address':
                    if otherinput.get() != "" and otherinput.get() != "xxxx.xxxx.xxxx":
                        ...
                        sleep(0.5)       
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'Interface MAC address configured.')
                    else:
                        tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập địa chỉ MAC để cấu hình trên giao diện.')
                elif otheroptn.get() == 'dot1q':
                    if "." not in getiface.get():
                        tkMessageBox.showinfo('Lỗi', 'Việc đóng gói Dot1q chỉ có thể được áp dụng cho một Giao diện con.')
                    elif otherinput.get() == "":
                        tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập ID VLAN để áp dụng trên Giao diện con.')
                    else:
                        sleep(0.5)
                        remote_conn.send("encap dot1Q " + otherinput.get() + "\n")
                        sleep(0.5)
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'Dot1q đã được cấu hình.')
                    ...
                    output = remote_conn.recv(buff_size).decode("utf-8")
                    if otherinput.get() == "":
                        tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập VRF để đăng ký trên giao diện.')
                    elif otherinput.get() not in output:
                        tkMessageBox.showinfo('Lỗi', 'Vui lòng tạo VRF trước (trong trang Lộ trình) trước khi áp dụng nó.')
                    else:
                        sleep(0.5)
                        remote_conn.send("ip vrf forwarding " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'VRF được định cấu hình nếu nó đã được tạo.')
                    ...
                    if 'tu' not in getiface.get():
                        tkMessageBox.showinfo('Lỗi', 'Chỉ có thể áp dụng fVRF cho các giao diện đường hầm.')
                    elif otherinput.get() == "":
                        tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập VRF để đăng ký trên giao diện.')
                    elif otherinput.get() not in output:
                        tkMessageBox.showinfo('Lỗi', 'Vui lòng tạo VRF trước (trong trang Lộ trình) trước khi áp dụng nó.')
                    ...
                    sleep(0.5)       
                    tkMessageBox.showinfo('Cài đặt giao diện khác', 'Được định cấu hình làm máy khách DHCP. BẤT KỲ ĐỊA CHỈ IP HIỆN TẠI ĐÃ BỊ XÓA.')
                elif otheroptn.get() == 'IP MTU(L3)':
                    if otherinput.get() != "" and otherinput.get() != "eg. 1400" and (68 <= int(otherinput.get()) <= 1500):
                        sleep(0.5)
                        remote_conn.send("ip mtu " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'Đã cấu hình gói giao diện MTU.')
                    else:
                        tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập giá trị MTU (68-1500) để áp dụng trên giao diện.')
                elif otheroptn.get() == 'MTU(L2)':
                    if otherinput.get() != "" and otherinput.get() != "eg. 1500" and (68 <= int(otherinput.get()) <= 1500):
                        sleep(0.5)
                        remote_conn.send("mtu " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'Configured interface frame MTU.')
                    else:
                        tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập giá trị MTU (1500-1530) để áp dụng trên giao diện.')                    
                elif otheroptn.get() == 'TCP MSS(L4)':
                    if otherinput.get() != "" and otherinput.get() != "eg. 1360" and (500 <= int(otherinput.get()) <= 1460):
                        ...
                        sleep(0.5)       
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'Configured MSS.')
                    else:
                        tkMessageBox.showinfo('Error', 'Vui lòng nhập giá trị MSS (500-1460) để áp dụng trên giao diện.')
                elif otheroptn.get() == 'VFR':
                    sleep(0.5)
                    remote_conn.send("ip virtual-reassembly \n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('Cài đặt giao diện khác', 'Đã bật IP Virtual Reassembly cho các gói bị phân mảnh.')
                elif otheroptn.get().encode('utf-8') == 'Thêm vào Bridge':
                    if otherinput.get() == "<bridge-grp no.>" or otherinput.get() == "":
                        tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập số Bridge Group.')
                    else:
                        sleep(0.5)
                        remote_conn.send("int " + getiface.get() + " \n")
                        sleep(0.5)
                        remote_conn.send("bridge-group " + otherinput.get() + "\n")
                        tkMessageBox.showinfo('Cài đặt giao diện khác', getiface.get() + ' đã được thêm vào nhóm cầu nối ' + otherinput.get())
                elif otheroptn.get() == 'tun key':
                    if getiface.get()[:2] != 'tu': 
                        tkMessageBox.showinfo('Lỗi', 'Khóa đường hầm chỉ có thể được áp dụng cho các giao diện đường hầm (ví dụ: tun0).')
                    elif otherinput.get() == "" or otherinput.get() == "eg. 1":
                        tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập giá trị khóa đường hầm để áp dụng trên giao diện.')
                    else:
                        remote_conn.send("tun key " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'Đã định cấu hình khóa đường hầm.')                
                elif otheroptn.get() == 'keepalive':
                    if otherinput.get() == "" or otherinput.get() == "<secs> [<retries>]":
                        tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập giá trị lưu giữ để áp dụng trên giao diện.')
                    elif getiface.get()[:2] != 'tu':
                        findfirstdigits = re.search(r'\d+', otherinput.get()).group()
                        sleep(0.2)
                        remote_conn.send("keepalive " + findfirstdigits + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'Interface keepalive đã được cấu hình.')
                    else:
                        remote_conn.send("keepalive " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'Tunnel interface keepalive đã được cấu hình.')
                elif otheroptn.get() == 'bandwidth':
                    if otherinput.get() == "" or otherinput.get() == "<kilobits>":
                        tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập giá trị băng thông để áp dụng trên giao diện.')
                    else:
                        remote_conn.send("bandwidth " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'Băng thông giao diện đã được cấu hình.')
                elif otheroptn.get() == 'IPSEC profile':
                    if getiface.get()[:2] != 'tu':
                        tkMessageBox.showinfo('Lỗi', 'Error - please apply IPSEC profile to a tunnel interface (e.g. tun0).')
                    elif otherinput.get() == "" or otherinput.get() == "<IPSec_prof_name>":
                        tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập hồ sơ IPSEC để đăng ký trên giao diện.')
                    else:                    
                        remote_conn.send("tunnel protection ipsec profile " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'Tunnel IPSEC profile đã được cấu hình.')                    
                else:
                    remote_conn.send(otheroptn.get() + " " + otherinput.get() + "\n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('Cài đặt giao diện khác', 'ARP timeout đã được cấu hình.')
            else:
                remote_conn.send("conf t \n")
                sleep(0.5)
                remote_conn.send("int " + getiface.get() + "\n")            
                if otheroptn.get() == 'MAC address':
                    ...
                    sleep(0.5)
                    tkMessageBox.showinfo('Cài đặt giao diện khác', 'Interface MAC address đã được đặt lại.')
                elif otheroptn.get() == 'dot1q':
                    if "." not in getiface.get():
                        tkMessageBox.showinfo('Lỗi', 'Không phải giao diện con.')
                    else:
                        sleep(0.5)
                        remote_conn.send("no encap dot1Q \n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'Dot1q đã bị xóa.')
                elif otheroptn.get() == 'VRF':
                    sleep(0.5)
                    remote_conn.send("no ip vrf forwarding \n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('Cài đặt giao diện khác', 'VRF đã bị xóa.')
                elif otheroptn.get() == 'DHCP Client':
                    sleep(0.5)
                    remote_conn.send("no ip addr dhcp \n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('Cài đặt giao diện khác', 'DHCP client đã bị xóa.')
                elif otheroptn.get() == 'IP MTU(L3)':
                    sleep(0.5)
                    remote_conn.send("no ip mtu \n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('Cài đặt giao diện khác', 'Gói giao diện MTU đặt lại thành giá trị tối đa mặc định (1500B).')
                elif otheroptn.get() == 'MTU(L2)':
                    sleep(0.5)
                    remote_conn.send("no mtu \n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('Cài đặt giao diện khác', 'Khung giao diện đặt lại MTU về mặc định tối đa (1500B).')                
                elif otheroptn.get() == 'TCP MSS(L4)':
                    sleep(0.5)
                    remote_conn.send("no ip tcp adjust-mss \n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('Cài đặt giao diện khác', 'Khung giao diện đặt lại MSS về mặc định tối đa (1460B).')
                elif otheroptn.get() == 'VFR':
                    sleep(0.5)
                    remote_conn.send("no ip virtual-reassembly \n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('Cài đặt giao diện khác', 'Đã tắt IP Virtual Reassembly cho các gói bị phân mảnh.')
                elif otheroptn.get().encode('utf-8') == 'Thêm vào Bridge':
                    sleep(0.5)
                    remote_conn.send("no bridge-group " + otherinput.get() + "\n")
                    tkMessageBox.showinfo('Cài đặt giao diện khác', getiface.get() + ' đã bị xóa khỏi Bridge Group ' + otherinput.get())
                elif otheroptn.get() == 'tun key':
                    if getiface.get()[:2] != 'tu': 
                        tkMessageBox.showinfo('Lỗi', 'Không phải giao diện đường hầm.')
                    else:
                        remote_conn.send("no tun key \n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'Tunnel key removed.')                
                elif otheroptn.get() == 'tun kpalive':
                    if getiface.get()[:2] != 'tu':
                        tkMessageBox.showinfo('Error', 'Không phải giao diện đường hầm.')
                    else:                    
                        remote_conn.send("no keepalive \n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'Tunnel keepalive đã bị xóa.')
                elif otheroptn.get() == 'bandwidth':
                        sleep(0.5)
                        remote_conn.send("no bandwidth \n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'Interface bandwidth đã bị xóa.')
                elif otheroptn.get() == 'IPSEC profile':
                    if getiface.get()[:2] != 'tu':
                        tkMessageBox.showinfo('Lỗi', 'Vui lòng chỉ định giao diện đường hầm (ví dụ: tun0).')
                    elif otherinput.get() == "":
                        tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập cấu hình IPSEC để xóa khỏi giao diện.')
                    else:                    
                        remote_conn.send("no tunnel protection ipsec profile " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('Cài đặt giao diện khác', 'Tunnel IPSEC profile đã bị xóa.') 
                else:
                    sleep(0.5)
                    remote_conn.send("no " + otheroptn.get() + " " + otherinput.get() + "\n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('Cài đặt giao diện khác', 'ARP timeout đã được đặt lại về mặc định (4 giờ).')
            remote_conn.send("exit \n")
            sleep(0.5)
            remote_conn.send("exit \n")                
    btn = Button(biface, text="*Áp dụng", bg="orange", command=setotherconf)
    ...
    def helpotherconf():
        res = 'VFR (Virtual Fragmentation Reassembly) - Có khả năng' +\
              ' phát hiện và ngăn chặn một loạt các cuộc tấn công phân mảnh. \n\nDot1q - Một gói được sử dụng cho Trunking, Bộ định tuyến trên thanh, v.v' +\
              ' Giá trị hợp lệ: 1-4094. Chỉ định cấu hình nó trên các giao diện con. Bất kỳ gói nào được chuyển đến địa chỉ IP của giao diện vật lý (nếu có cấu hình)' +\
              ' sẽ được bôi đen..' +\
              '\n\nVRF - Hoạt động giống như một VLAN lớp 3. Nó PHẢI được tạo (trong tab Lộ trình) trước khi có thể được áp dụng.\n\n' +\
              'IP phụ - Bạn có thể cấu hình nhiều IP phụ trên một giao diện. Nếu không có IP chính được định cấu hình, IP phụ sẽ ' +\
              'vẫn không hoạt động và chỉ ẩn cho đến khi một IP chính được cấu hình.' +\
              '\n\nTới cổng bộ định tuyến Bridge, \n 1. Tạo Bridge Group,\n 2. Tạo giao diện BVI\n 3. Cài đặt khác > "Thêm vào Bridge"'
        tkMessageBox.showinfo('Trợ giúp cài đặt giao diện khác', res)
    btn = Button(biface, text="Gợi ý", bg="yellow", command=helpotherconf)
    ...
    deviceconfframe=LabelFrame(window,text=" Cấu hình thiết bị và điều chỉnh ",font=('verdana', 8, 'bold'),padx=10,pady=8,width=100,height=100)
    deviceconfframe.grid(row=4,column=0, sticky=("nsew"))

    label = Label(deviceconfframe, text="Tạo mục nhập ARP tĩnh: ").grid(column=0, row=1)
    ...
    def setarp():
        ...
        if chk_state_neg.get() == False:
            ...
            remote_conn.send("exit \n")
            sleep(0.5)
            tkMessageBox.showinfo('ARP', 'ARP tĩnh đã được định cấu hình.')
        else:
            remote_conn.send("no arp " + getarpip.get() + " " + getarpmac.get() + " arpa \n")
            sleep(0.5)
            remote_conn.send("exit \n")
            sleep(0.5)
            tkMessageBox.showinfo('ARP', 'ARP tĩnh đã bị xóa.')          
    btn = Button(deviceconfframe, text="Áp dụng", font=('helvetica', 8), bg="orange", command=setarp)
    btn.grid(column=3, row=1, padx=13)

    label = Label(deviceconfframe, text="Tạo mục nhập MAC tĩnh: ").grid(column=0, row=2)
    ...
    getstvlan.insert(0, "VLAN tùy chọn")
    def handle_focus_in(_):
    ...
    def setmac():
        if getiface.get() == '' or getiface.get() == 'e.g. fa0/0, fa0/0.2, vlan1, tun0, bvi1, po1 ..':
            ...
            sleep(0.5)
            if chk_state_neg.get() == False:
                if getstvlan.get() == '' or getstvlan.get().encode('utf-8') == 'VLAN tùy chọn':
                    remote_conn.send("mac-address-table static " + getstmac.get() + " interface " + getiface.get() + " \n")
                    sleep(0.5)
                    remote_conn.send("exit \n")
                    sleep(0.5)
                    tkMessageBox.showinfo('MAC', 'Đã cấu hình MAC tĩnh. Lưu lượng mạng hướng đến đích ' + getstmac.get() + ' sẽ được chuyển đến ' + getiface.get())
                else:
                    remote_conn.send("mac-address-table static " + getstmac.get() + " interface " + getiface.get() + " vlan " + getstvlan.get() + " \n")
                    sleep(0.5)
                    remote_conn.send("exit \n")
                    sleep(0.5)
                    tkMessageBox.showinfo('MAC', 'MAC tĩnh được định cấu hình. Lưu lượng mạng trong ' + getstvlan.get() +  ' đến đích ' + getstmac.get() + \
                                          ' sẽ được chuyển đến ' + getiface.get())
            else:
                if getstvlan.get() == '' or getstvlan.get().encode('utf-8') == 'VLAN tùy chọn':
                    ...
                    remote_conn.send("exit \n")
                    sleep(0.5)
                    tkMessageBox.showinfo('MAC', 'MAC tĩnh đã bị xóa.')
                else:
                    ...
                    sleep(0.5)
                    tkMessageBox.showinfo('MAC', 'MAC tĩnh đã bị xóa.')        
    btn = Button(deviceconfframe, text="*Áp dụng", font=('helvetica', 8), bg="orange", command=setmac)
    btn.grid(column=3, row=2, padx=13, pady=2) 


    def option_changed_arpmacopt(*args):
        if arpmacopt.get().encode('utf-8') == "Xóa ARP (all)" or arpmacopt.get().encode('utf-8') == "Xóa MAC (all)":
            text = 'NoInputRequired'
        ...
        elif arpmacopt.get().encode('utf-8') == "Xóa ARP (int)" or arpmacopt.get().encode('utf-8') == "Xóa MAC (int)":
            text = 'eg. fa0/0'
            getmacarp.delete(0, END)
            getmacarp.config(fg='grey')
            getmacarp.insert(0, text)
            ...
            getmacarp.bind("<FocusOut>", handle_focus_out)
            getmacarp.bind("<FocusIn>", handle_focus_in)
        elif arpmacopt.get().encode('utf-8') == "Xóa ARP (ip)":
            text = 'eg. 1.2.3.4'
            getmacarp.delete(0, END)
            getmacarp.config(fg='grey')
            getmacarp.insert(0, text)
            ...
            getmacarp.bind("<FocusOut>", handle_focus_out)
            getmacarp.bind("<FocusIn>", handle_focus_in)
        elif arpmacopt.get().encode('utf-8') == "Xóa MAC (addr)":
            text = 'xxxx.xxxx.xxxx'
            getmacarp.delete(0, END)
            getmacarp.config(fg='grey')
            getmacarp.insert(0, text)
            ...
            getmacarp.bind("<FocusOut>", handle_focus_out)
            getmacarp.bind("<FocusIn>", handle_focus_in)
        elif arpmacopt.get().encode('utf-8') == "Xóa MAC (vlan)":
            text = '<1-4094>'
            getmacarp.delete(0, END)
            getmacarp.config(fg='grey')
            getmacarp.insert(0, text)
    ...
    def arpmacclearset():
        if arpmacopt.get().encode('utf-8') == "Xóa ARP (all)":
            remote_conn.send("clear arp \n")
            sleep(0.5)
            tkMessageBox.showinfo('Bảng ARP', 'Tất cả các mục trong bảng ARP đã bị xóa.')
        elif arpmacopt.get().encode('utf-8') == "Xóa ARP (ip)":
            if getmacarp.get() != "" and ("." in getmacarp.get()):
                remote_conn.send("clear ip arp " + getmacarp.get() + "\n")
                sleep(0.5)
                tkMessageBox.showinfo('Bảng ARP', 'Các mục nhập bảng ARP đã được xóa với thuộc tính IP được chỉ định.')
            else:
                tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập địa chỉ IP để xóa mục nhập ARP liên quan.')
        elif arpmacopt.get().encode('utf-8') == "Xóa ARP (int)":
            ...
            ("tun" in getmacarp.get()) or ("lo" in getmacarp.get())):
                ...
                sleep(0.5)
                tkMessageBox.showinfo('Bảng ARP', 'Các mục nhập bảng ARP đã được xóa với thuộc tính giao diện được chỉ định.')
            else:
                tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập giao diện (CHỈ fa / gi / te / tun / lo / bvi) để xóa mục nhập ARP liên quan.')
        elif arpmacopt.get().encode('utf-8') == "Xóa MAC (all)":
                remote_conn.send("clear mac-address-table \n")
                sleep(0.5)
                tkMessageBox.showinfo('Bảng MAC', 'Tất cả các mục trong bảng địa chỉ MAC đã bị xóa.')
        elif arpmacopt.get().encode('utf-8') == "Xóa MAC (vlan)":
            if getmacarp.get() != "" and getmacarp.get().isdigit() == True:
                remote_conn.send("clear mac-address-table vlan " + getmacarp.get() + " \n")
                sleep(0.5)
                tkMessageBox.showinfo('Bảng MAC', 'Tất cả các mục nhập bảng địa chỉ MAC đã được xóa cho VLAN ' + getmacarp.get() + '.')
            else:
                tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập ID VLAN để xóa mục nhập địa chỉ MAC được liên kết.')
        elif arpmacopt.get().encode('utf-8') == "Xóa MAC (addr)":
            if getmacarp.get() != "" and ("." in getmacarp.get()):
                remote_conn.send("clear mac-address-table address " + getmacarp.get() + " \n")
                sleep(0.5)
                tkMessageBox.showinfo('Bảng MAC', 'Mục nhập bảng địa chỉ MAC đã bị xóa.')
            else:
                tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập Địa chỉ MAC (vv. XXXX.XXXX.XXXX) để xóa mục nhập địa chỉ MAC được liên kết.')
        elif arpmacopt.get().encode('utf-8') == "Xóa MAC (int)":
            if getmacarp.get() != "":
                remote_conn.send("clear mac-address-table int " + getmacarp.get() + " \n")
                sleep(0.5)
                tkMessageBox.showinfo('Bảng MAC', 'Các mục trong bảng địa chỉ MAC đã bị xóa đối với giao diện ' + getmacarp.get() + '.')
            else:
                tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập một giao diện (CHỈ fa / gi / te / tun / lo / bvi) để xóa mục nhập địa chỉ MAC liên quan.')
        else:
            if chk_state_neg.get() == False:
                if getmacarp.get() != "" and getmacarp.get().isdigit() == True and (getmacarp.get() <= 1000000):
                    ...
                    tkMessageBox.showinfo('Bảng MAC', 'Thời gian lão hóa bảng địa chỉ MAC đã được cấu hình.')
                else:
                    tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập thời gian lão hóa địa chỉ MAC bằng số hợp lệ.')
            else:
                remote_conn.send("no mac-address-table aging \n")
                sleep(0.5)
                tkMessageBox.showinfo('Bảng MAC', 'Thời gian lão hóa bảng địa chỉ MAC đã được đặt lại.')
    btn = Button(deviceconfframe, text="Áp dụng", font=('helvetica', 8), bg="orange", command=arpmacclearset)
        ...
        if chk_state_neg.get() == False:
            if TCPtuneopt.get() == "PMTUD":
                ...
                if getTCPtune.get() != "" and getTCPtune.get() != "[<2-20> (mins)]":
                    ...
                    sleep(0.3)
                    remote_conn.send("exit \n")
                    tkMessageBox.showinfo('TCP', 'PMTUD đã được định cấu hình với việc hiệu chuẩn lại MTU cứ sau ' + getTCPtune.get() + ' phút.')
                else:
                    remote_conn.send("ip tcp path-mtu-discovery \n")
                    sleep(0.3)
                    remote_conn.send("exit \n")
                    tkMessageBox.showinfo('TCP', 'PMTUD đã được định cấu hình.')                 
            elif TCPtuneopt.get() == "TCP egress queue":
                if getTCPtune.get() != "" and getTCPtune.get() != "eg. 5 (segments)":
                    ...
                    sleep(0.3)
                    remote_conn.send("exit \n")
                    tkMessageBox.showinfo('TCP', 'TCP egress queue đã được định cấu hình.')
                else:
                    tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập một giá trị.')                    
            elif TCPtuneopt.get() == "SYNWAIT-time":
                if getTCPtune.get() != "" and getTCPtune.get() != "eg. 30 (secs)":
                    ...
                    sleep(0.3)
                    remote_conn.send("exit \n")
                    tkMessageBox.showinfo('TCP', 'TCP SYNWAIT-time đã được định cấu hình.')
                else:
                    tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập một giá trị.') 
            elif TCPtuneopt.get() == "TCP window-size":
                if getTCPtune.get() != "" and getTCPtune.get() != "eg. 65535": 
                    ...
                    sleep(0.3)
                    remote_conn.send("exit \n")
                    tkMessageBox.showinfo('TCP', 'TCP window-size đã được định cấu hình.')
                else:
                    tkMessageBox.showinfo('Lỗi', 'Vui lòng nhập một giá trị.') 
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP Explicit Congestion Notification tính năng đã được cấu hình. LƯU Ý: tính năng ngang hàng từ xa cũng phải được bật ECN để tính năng này hoạt động.')
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP Selective-ACK(SACK) đã được định cấu hình.')
            else:
                remote_conn.send("conf t \n")
                sleep(0.3)
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP timestamp đã được định cấu hình.')
        else:
            if TCPtuneopt.get() == "PMTUD":
                remote_conn.send("conf t \n")
                sleep(0.3)
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP PMTUD đã bị vô hiệu hóa.') 
            elif TCPtuneopt.get() == "TCP egress queue":
                remote_conn.send("conf t \n")
                sleep(0.3)
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP egress-queue-max đã được đặt lại về mặc định.') 
            elif TCPtuneopt.get() == "SYNWAIT-time":
                remote_conn.send("conf t \n")
                sleep(0.3)
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP SYNWAIT-time đã được đặt lại về mặc định.') 
            elif TCPtuneopt.get() == "TCP window-size":
                remote_conn.send("conf t \n")
                sleep(0.3)
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP window-size đã được đặt lại về mặc định.') 
            elif TCPtuneopt.get() == "Enable ECN":
                remote_conn.send("conf t \n")
                sleep(0.3)
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP Explicit Congestion Notification đã bị vô hiệu hóa.') 
            elif TCPtuneopt.get() == "Enable Selective-ACK":
                remote_conn.send("conf t \n")
                sleep(0.3)
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP Selective-ACK đã bị vô hiệu hóa.')
            else:
                remote_conn.send("conf t \n")
                sleep(0.3)
                remote_conn.send("ip tcp timestamp \n")
                sleep(0.3)
                remote_conn.send("exit \n")                
            tkMessageBox.showinfo('TCP', 'TCP timestamp đã bị vô hiệu hóa.')
    btn = Button(deviceconfframe, text="Áp dụng", font=('helvetica', 8), bg="orange", command=TCPtune)
    ...
    chk = Checkbutton(deviceconfframe, text="Học MAC", font=("TkDefaultFont", 8), variable=chk_MAClearn)
    ...
    btn = Button(deviceconfframe, text="Áp dụng", font=('helvetica', 8), bg="orange", command=createbridgeG)
    ...
    showconfframe=LabelFrame(window,text=" Hiển thị cấu hình ",font=('verdana', 8, 'bold'),padx=50,pady=8,width=100,height=100)
    showconfframe.grid(row=6,column=0, sticky=("nsew"))
    
    OPTIONS = [
    "Thống kê giao diện",
    "Cấu hình giao diện",
    "Thống kê giao diện L3",
    "Tóm tắt giao diện"
    ]
    ...
    def showintconf():
        if showintopt.get().encode('utf-8') == "Thống kê giao diện":
            if getiface.get() == '' or getiface.get() == 'e.g. fa0/0, fa0/0.2, vlan1, tun0, bvi1, po1 ..':
                tkMessageBox.showinfo('Lỗi', 'Vui lòng chọn một giao diện.')
            else:
                buff_size = 16384
                ...
                while not remote_conn.recv_ready():
                    ...
                    sleep(0.5)
                output = remote_conn.recv(buff_size).decode("utf-8")
                tkMessageBox.showinfo('Cấu hình giao diện hiện tại', output)
        elif showintopt.get().encode('utf-8') == "Cấu hình giao diện":
            if getiface.get() == '' or getiface.get() == 'e.g. fa0/0, fa0/0.2, vlan1, tun0, bvi1, po1 ..':
                tkMessageBox.showinfo('Lỗi', 'Vui lòng chọn một giao diện.')
            else:
                rep = getiface.get()
                ...
                output = remote_conn.recv(buff_size).decode("utf-8")
                tkMessageBox.showinfo('Cấu hình giao diện hiện tại', output)
        elif showintopt.get().encode('utf-8') == "Thống kê giao diện L3":
            if getiface.get() == '' or getiface.get() == 'e.g. fa0/0, fa0/0.2, vlan1, tun0, bvi1, po1 ..':
                tkMessageBox.showinfo('Lỗi', 'Vui lòng chọn một giao diện.')
            else:
                buff_size = 16384
            ...
            output = remote_conn.recv(buff_size).decode("utf-8")
            tkMessageBox.showinfo('Cấu hình giao diện hiện tại', output)        
    btn = Button(showconfframe, text="Hiển thị", bg="orange", command=showintconf)
    btn.place(x=300, y=0)
    ...
    "CPU",
    "Ký ức",
    "Song công / Tốc độ (Đã cấu hình)",
    "Song công / Tốc độ (Hoạt động)",
    "Kết nối TCP",
    "Thống kê TCP",
    "UDP",
    "Các cổng nghe",
    ...
    def showintconf2():
        if showintopt2.get().encode('utf-8') == "Bảng MAC":
            buff_size = 16384
            sleep(0.5)
            ...
            output = remote_conn.recv(buff_size).decode("utf-8")
            tkMessageBox.showinfo('Cấu hình giao diện hiện tại', output)
        elif showintopt2.get().encode('utf-8') == "Bảng MAC (tổng số)":
            buff_size = 16384
            sleep(0.5)
            ...
            output = remote_conn.recv(buff_size).decode("utf-8")
            tkMessageBox.showinfo('Cấu hình giao diện hiện tại', output)
        elif showintopt2.get().encode('utf-8') == "Bảng ARP":
            buff_size = 16384
            sleep(0.5)
            ...
            if showintopt2.get().encode('utf-8') == "Tình trạng phần cứng":
                cmd = "show env \n"
            elif showintopt2.get().encode('utf-8') == "Trạng thái phần cứng (chi tiết)":
                cmd = "show env all \n"                
            elif showintopt2.get().encode('utf-8') == "Thông tin phần cứng":
                cmd = "show inv \n"
            elif showintopt2.get().encode('utf-8') == "Trạng thái Buffer":
                cmd = "show buff \n"
            elif showintopt2.get().encode('utf-8') == "Đồng hồ":
                cmd = "show clock \n"
            elif showintopt2.get().encode('utf-8') == "Thời gian hoạt động":
                cmd = "show ver | i uptime | returned | restarted \n"
            elif showintopt2.get().encode('utf-8') == "Phiên bản":
                cmd = "show ver | s IOS \n"
            elif showintopt2.get() == "CPU":
                cmd = "show proc cpu hist \n"
            elif showintopt2.get().encode('utf-8') == "Ký ức":
                cmd = "show proc mem \n"
            elif showintopt2.get().encode('utf-8') == "Song công / Tốc độ (Đã cấu hình)":
                cmd = "show run | i interface | duplex | speed \n"
            elif showintopt2.get().encode('utf-8') == "Song công / Tốc độ (Hoạt động)":
                cmd = "show int | i dup| up \n"                
            elif showintopt2.get().encode('utf-8') == "Kết nối TCP":
                cmd = "show tcp brief nume \n"
            elif showintopt2.get().encode('utf-8') == "Thống kê TCP":
                cmd = "show tcp stat \n"
            elif showintopt2.get().encode('utf-8') == "Các cổng nghe":
            ...
            output = remote_conn.recv(buff_size).decode("utf-8")
            tkMessageBox.showinfo('Cấu hình giao diện hiện tại', output)
    btn = Button(showconfframe, text="Hiển thị", bg="orange", command=showintconf2)
...
if __name__ == "__main__":
        ...
        main()
        window.withdraw()
        jumphostip = askstring("", "Nhập IP của jumphost (nếu không có, hãy nhập 'n'):")
        ip = askstring("", "Nhập IP máy chủ:")
        if jumphostip == "" or ip == "":
            window.destroy()
