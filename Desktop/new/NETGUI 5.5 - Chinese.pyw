...
localip = "172.30.112.214"    #Syslog svr的IP地址（用于“调试”功能）
...
def loginjumphost():
    ...
    try:
        ...
        tkMessageBox.showinfo('错误', '连接超时; 无法连接到设备.')
        quit()
    ...
    def route():
        ...
        def staticro():
            ...
            staticgw.get() == "gateway IP":
                tkMessageBox.showinfo('错误', 'Please input the minimum required information - IP, subnet mask and next hop gateway.', parent=window)
            else:
                    ...
                    if chk_state_mcast.get() == False:
                        ...
                        if (Trackopt.get() != "Track" and Trackopt.get() != "") or (Nameopt.get() != "Name" and Nameopt.get() != "")\
                           or (Tagopt.get() != "Tag" and Tagopt.get() != ""):
                            tkMessageBox.showinfo('错误', 'Track, name and tags cannot be used with multicast routes.', parent=window)
                        else:                            
                            if VRFopt.get() == "" or VRFopt.get() == "VRF":
                                if Metricopt.get() != "" and Metricopt.get() != "Metric":
        ...
        def enospf():
            ...
            if (a == '' or a == 'eg. 15(MyVRF)'):
                tkMessageBox.showinfo('错误', 'Please enter a OSPF PID (and VRF if required).', parent=window)
            else:
                if "(" in txtospfpid.get():
                    ...
                    if "No VRF" in stripp:
                        sleep(0.2)
                        tkMessageBox.showinfo('错误', 'VRF not found.. Please create the VRF first. Aborting.', parent=window)
                        return
                    else:
                        pass
                else:
                ...
                if chk_state_neg.get() == False:
                    if (b == '') and ((c == '') or (c == 'eg. 0.0.0.255')) and (d == ''):
                        ...
                        tkMessageBox.showinfo('错误', 'Please enter area, route and mask to advertise; else, leave all 3 blank to enable an OSPF process.', parent=window)                        
                else:  
                    if b == '' and c == ''  and d == '':
                        remote_conn.send("conf t\n")
                        sleep(0.2)
                        ...
                        tkMessageBox.showinfo('错误', 'Please enter area, route and mask to unadvertise; else, leave all 3 blank to disable an OSPF process.', parent=window)
        ...
        def ospfifaceopt():
            ...
            if ospfoptiface.get() == "" or ospfoptiface.get() == "eg. fa0/0":
                tkMessageBox.showinfo('错误', 'Please enter an interface first.', parent=window)
            else:
                remote_conn.send("conf t \n")
                sleep(0.2)
                if chk_state_neg.get() == False:
                    if getOSPFoptiface.get() == "Hello timer":                        
                        ...
                        if txtospfpid.get() == "" or txtospfpid.get() == "eg. 15(MyVRF)":
                            tkMessageBox.showinfo('错误', 'Please also enter the OSPF process ID above.', parent=window)
                        else:
                            if "(" not in txtospfpid.get():
                                remote_conn.send(defospfcmd)
                            else:
                        ...
                        if txtospfpid.get() == "" or txtospfpid.get() == "eg. 15(MyVRF)" or txtospfarea.get() == "" or ("(" in txtospfpid.get()):
                            tkMessageBox.showinfo('错误', 'Please also enter both the OSPF process ID (non-VRF only) and area above.', parent=window)
                        else:
                            remote_conn.send("int " + ospfoptiface.get() + " \n")
                            sleep(0.2)
            ...
            if txtospfpid.get() == "" or txtospfpid.get() == "eg. 15(MyVRF)":
                ...
                if chk_state_neg.get() == False:                
                    if getOSPFoptdevice.get() == "Router-ID":
                        ...
                        if txtospfarea.get() == "":
                            tkMessageBox.showinfo('错误', 'Please also enter an OSPF area above.', parent=window)
                        else:
                            if ospfopt2.get() == "Stub":
                                ...
                                tkMessageBox.showinfo('错误', 'Please enter ONE stub type to configure the area.', parent=window)
                    elif getOSPFoptdevice.get() == "Virt link":
                        if txtospfarea.get() == "":
                            tkMessageBox.showinfo('错误', 'Please also enter an OSPF area above.', parent=window)
                        else:
        ...
        def eneigrp():
            ...
            if (d == '' or d == 'eg. 15(MyVRF)AS1'):
                tkMessageBox.showinfo('错误', 'Please enter minimum required information (EIGRP PID).', parent=window)
            elif ("(" in d) and ("AS" not in d):
                tkMessageBox.showinfo('错误', 'Please enter both the EIGRP PID VRF and AS number.', parent=window)
                ...
                if chk_state_neg.get() == False:
                    if (e == '') and ((f == '') or (f == 'eg. 0.0.0.255')):
                        ...
                        tkMessageBox.showinfo('错误', 'Please enter both route and mask to advertise.', parent=window)
                    else:
                        remote_conn.send("conf t\n")
                        sleep(0.5)
                        if "(" not in txteigrppid.get():
                    ...
                    if (e == '') and ((f == '') or (f == 'eg. 0.0.0.255')):
                        ...
                        tkMessageBox.showinfo('错误', 'Please enter both route and mask to unadvertise.', parent=window)
                    else:
                        remote_conn.send("conf t\n")
                        sleep(0.5)
                        if "(" not in txteigrppid.get():
        ...
        def eigrpifaceopt():
            ...
            if txteigrppid.get() == "" or txteigrppid.get() == "eg. 15(MyVRF)AS1":
                tkMessageBox.showinfo('错误', 'Please also enter the EIGRP PID above.', parent=window)
                ...
                if chk_state_neg.get() == False:
                    if getEIGRPoptiface.get() == "Hello timer":
                        if eigrpopt1.get() == "" or eigrpopt1.get() == "eg. 5":
                            tkMessageBox.showinfo('错误', 'Please enter a Hello timer value.', parent=window)
                        else:
                        ...
                        if eigrpopt1.get() == "":
                            tkMessageBox.showinfo('错误', 'Please enter a Hold timer value.', parent=window)
                        else:
                            remote_conn.send("int " + eigrpoptiface.get() + " \n")
                            sleep(0.2)                
                        ...
                        if eigrpopt1.get() == "" or eigrpopt1.get() == "<neigh_IP>":
                            tkMessageBox.showinfo('错误', 'Please enter a neighbor IP address.', parent=window)
                        else:
                        ...
                        if eigrpopt1.get() == "" or eigrpopt1.get() == "eg. 10000 (Kbps)":
                            tkMessageBox.showinfo('错误', 'Please enter a bandwidth value.', parent=window)
                        else:
                            remote_conn.send("int " + eigrpoptiface.get() + " \n")
                            sleep(0.2)
                        ...
                        if eigrpopt1.get() == "" or eigrpopt1.get() == "<net> <mask> [<AD>]":
                            tkMessageBox.showinfo('错误', 'Please enter a summary-address <<ip> <mask> [AD]> value.', parent=window)
                        else:
                            remote_conn.send("int " + eigrpoptiface.get() + " \n")
                            sleep(0.2)
                        ...
                        if eigrpopt1.get() == "" or eigrpopt1.get() == "<acl_no.> | prefix-l <> + i/o":
                            tkMessageBox.showinfo('错误', 'Please specify the <<ACL/prefix<pfxlist>> <in/out>>.', parent=window)
                        else:
                            if "(" not in txteigrppid.get():
                                remote_conn.send(defeigrpcmd)
                            else:
                        ...
                        if eigrpopt1.get() == "" or eigrpopt1.get() == "<acl_no.> + i/o + offset_val":
                            tkMessageBox.showinfo('错误', 'Please specify the <<ACL> <in/out> <metric-offset-val>>.', parent=window)
                        else:
                            if "(" not in txteigrppid.get():
                                remote_conn.send(defeigrpcmd)
                            else:
                        ...
                        if eigrpopt1.get() == "" or eigrpopt1.get() == "<keychain_name>":
                            tkMessageBox.showinfo('错误', 'Please specify a Keychain name.', parent=window)
                        else:
                            remote_conn.send("int " + eigrpoptiface.get() + " \n")
                            sleep(0.2)
                    ...
                    if getEIGRPoptiface.get() == "Hello timer":                        
                        ...
                        if eigrpopt1.get() == "" or eigrpopt1.get() == "<acl_no.> + i/o + offset_val":
                            tkMessageBox.showinfo('错误', 'Please specify the <<ACL> <in/out>>.', parent=window)
                        else:
                            if "(" not in txteigrppid.get():
                                remote_conn.send(defeigrpcmd)
                            else:
                ...
                if chk_state_neg.get() == False:
                    if getEIGRPoptdevice.get() == "Router-ID":
                        if eigrpopt2.get() == "" or eigrpopt2.get() == "eg. 1.1.1.1":
                            tkMessageBox.showinfo('错误', 'Please enter Router ID (IP) value.', parent=window)
                        else:
...
above to propagate at interface level\n - ', parent=window)                        
                        ...
                        if eigrpopt2.get() == "" or eigrpopt2.get() == "<1-128>":
                            tkMessageBox.showinfo('错误', 'Please enter variance (1-128) value.', parent=window)
                        else:
                        ...
                        if eigrpopt2.get() == "" or eigrpopt2.get() == "<1-16>":
                            tkMessageBox.showinfo('错误', 'Please enter a value.', parent=window)
                        else:
                        ...
                        if eigrpopt2.get() == "" or eigrpopt2.get() == "<1-255> [<1-255>]":
                            tkMessageBox.showinfo('错误', 'Please enter AD values.', parent=window)
                        else:
                        ...
                        if eigrpopt2.get() == "" or eigrpopt2.get() == "<AD> <net> <wildcard> [acl_no.]":
                            tkMessageBox.showinfo('错误', 'Please enter required values.')
                        else:
                            remote_conn.send("distance " + eigrpopt2.get() + " \n")
                            tkMessageBox.showinfo('EIGRP', 'Distance has been configured.', parent=window)
                    else:
                        if eigrpopt2.get() == "" or eigrpopt2.get() == "<0-255> <0-255> <0-255> <0-255> <0-255>":
                            tkMessageBox.showinfo('错误', 'Please enter all 5 (K1-K5) weight values.', parent=window)
                        else:
...
above to propagate at interface level\n - ', parent=window)                        
                        ...
                        if eigrpopt2.get() == "" or eigrpopt2.get() == "<AD> <net> <wildcard> [acl_no.]":
                            tkMessageBox.showinfo('错误', 'Please enter required values.', parent=window)
                        else:
        ...
        def eigrpredistribute():
            ...
            if chk_state_neg.get() == False:
                if txteigrppid.get() == "" or txteigrppid.get() == "eg. 15(MyVRF)AS1":
                    tkMessageBox.showinfo('错误', 'Please also enter the EIGRP process ID above.', parent=window)
                elif "(" in txteigrppid.get() and "AS" not in txteigrppid.get():
                    tkMessageBox.showinfo('错误', 'Please enter both the EIGRP PID VRF and AS number.', parent=window)
                elif "AS" in txteigrppid.get() and "(" not in txteigrppid.get():
                    tkMessageBox.showinfo('错误', 'Please enter both the EIGRP PID VRF and AS number.', parent=window)
                    ...
                    if eigrpredistMetric.get() != "" and eigrpredistMetric.get() != "Metric":
                        ...
                        if len(match) != 5:
                            tkMessageBox.showinfo('错误', 'Please enter proper EIGRP Metric (bandwidth, delay, reliability, load, MTU) seperated by spaces: e.g. 1000 100 255 1 1500', parent=window)
                            return
                        else:
                            pass
                    else:
        ...
        def enbgp():
            ...
            if chk_state_neg.get() == False:
                if (i == '') or (i == 'eg. 15(MyVRF)'):
                    tkMessageBox.showinfo('错误' + bgpproc.get(), '错误 - Please enter a BGP PID (AS number)', parent=window)
                else:
                    if "(" in bgpproc.get():
                    ...
                    if 'is "bgp' in stripp and ((("(" not in bgpproc.get()) and (i not in checkbgp)) or (("(" in bgpproc.get()) and \
                        ...
                        sleep(0.5)
                        tkMessageBox.showinfo('错误', 'BGP AS ' + checkbgp + ' already exists. Only 1 BGP AS/process can be running at any time.', parent=window)
...
##                            tkMessageBox.showinfo('BGP ' + i, 'This BGP process is already running', parent=window)
                        ...
                        if AFIvar.get() == 1:
                            if "(" not in bgpproc.get():
                                ...
                                remote_conn.send("exit \n")
                                sleep(0.2)
                                tkMessageBox.showinfo('错误', 'BGP enabled, AF is not. Please select either an IPv4 or IPv6 AF to apply VRFs.', parent=window)
                                sleep(0.2)
                                return
                        elif AFIvar.get() == 2:
                            if "(" not in bgpproc.get():
                            ...
                            if "(" not in bgpproc.get():
                                ...
                                if "not enabled" in stripp:
                                    ...
                                    remote_conn.send("exit \n")
                                    sleep(0.2)
                                    tkMessageBox.showinfo('错误', 'IPv6 not running.. Please enable it first. Aborting.', parent=window)
                                    return
                                else:
                                    pass
                                sleep(0.2)
...
##                            bgpcomment = 'BGP VRF enabled.'
                ...
                if (i == '') or (i == 'eg. 15(MyVRF)'):
                    ...
                    if 'is "bgp' in stripp and ("(" not in bgpproc.get()) and (i not in checkbgp):
                        ...
                        sleep(0.5)
                        tkMessageBox.showinfo('错误', bgprunningAS + ' already exists. Please input correct AS number to remove.', parent=window)
        ...
        def bgpotheropt():
            ...
            if chk_state_neg.get() == False:
                if (i == '') or (i == 'eg. 15(MyVRF)') or (j == '') or (j == 'Neighbor IP'):
                    tkMessageBox.showinfo('错误' + bgpproc.get(), '错误 - Please enter a BGP PID (AS number) and Neighbor IP', parent=window)
                else:
                    remote_conn.send("sh ip proto | i bgp \n")
                    sleep(0.5)
                    output = remote_conn.recv(2048).decode("utf-8")
                ...
                if (i == '') or (i == 'eg. 15(MyVRF)') or (j == '') or (j == 'Neighbor IP'):
                    ...
                    if 'is "bgp' in stripp and ((("(" not in bgpproc.get()) and (i not in checkbgp)) or (("(" in bgpproc.get()) and \
                        ...
                        if getBGPoptdevice.get() == "Router-ID":
                            ...
                            tkMessageBox.showinfo('BGP', '错误 / WARNING: This command has been removed for safety; deactivating an AF will also\
delete all config associated along with that specific AF.', parent=window)
            ...
            if chk_state_neg.get() == False:
                if getBGPoptredist.get() == "RIP" or getBGPoptredist.get() == "EIGRP" or getBGPoptredist.get() == "BGP":
                    if bgpproc.get() == "" or bgpproc.get() == "eg. 15(MyVRF)":
                        tkMessageBox.showinfo('错误', 'Please also enter the BGP process ID above.', parent=window)
                    else:
                        remote_conn.send("conf t \n")
                        sleep(0.2)
                        if "(" not in bgpproc.get():
                        ...
                        if AFIvar.get() == 1:
                            if "(" not in bgpproc.get():
                                ...
                                remote_conn.send("exit \n")
                                sleep(0.2)
                                tkMessageBox.showinfo('错误', 'Please select either an IPv4 or IPv6 AF to apply VRFs.', parent=window)
                                return
                        elif AFIvar.get() == 2:
                            if "(" not in bgpproc.get():
                                remote_conn.send("address-f ipv4 " + SAFIopt.get() + " \n")
        ...
        def createvrf():
            if chk_state_neg.get() == False:
                if getVRF.get() == '' :
                    tkMessageBox.showinfo('错误', 'Please enter a VRF name.', parent=window)
                    return
                    ...
                    if ":" not in getVRFrd.get():
                        tkMessageBox.showinfo('错误', 'Please enter in correct format with colon (e.g. 100:200).', parent=window)
                        return
                    else:
                        pass
                    remote_conn.send("conf t\n")
        ...
        def showroute():
            if showrouteopt.get() == "Active routes":
                ...
                if showopt.get() == "" or showopt.get() == "eg. eigrp, ospf 2":
                    tkMessageBox.showinfo('错误', 'Please enter a routing protocol.', parent=window)
                else:
                    buff_size = 16384
                    sleep(0.5)
        ...
        def showproto():
            if showprotopt.get() == "RIP":
                if showopt2.get() == "" or showopt2.get() == "[database | neigh [<>]]":
                    tkMessageBox.showinfo('错误', 'Please enter an option.', parent=window)
                else:
                    buff_size = 16384
                    sleep(0.5)
                ...
                if showopt2.get() == "":
                    tkMessageBox.showinfo("错误", "Please enter BGP neighbor's IP address to view (filtered) routes advertised to it.", parent=window)
                else:
                    buff_size = 16384
                    sleep(0.5)
                ...
                if showopt2.get() == "":
                    tkMessageBox.showinfo("错误", "Please enter BGP neighbor's IP address to view (before filtering) routes advertised to it.", parent=window)
                else:
                    buff_size = 16384
                    sleep(0.5)
                ...
                if showopt2.get() == "":
                    tkMessageBox.showinfo("错误", "Please enter BGP neighbor's IP address to view routes received (before filtering) from it.", parent=window)
                else:
                    buff_size = 16384
                    sleep(0.5)
    ...
    def switch():
        ...
        def createvlan():
            if createv.get() == "" or createv.get() == "e.g. 1, 4094, 3-15":
                tkMessageBox.showinfo('错误', 'Please enter VLAN ID.', parent=window)
            else:
                if chk_state_neg.get() == False:
                    remote_conn.send("conf t\n")
                    sleep(0.2)
        ...
        def setvlan():
            ...
            or getiface.get() == "" or getiface.get() == "e.g. Eth0/0":
                    tkMessageBox.showinfo('错误', 'Please enter VLAN ID, interface, and choose switchport mode.', parent=window)
            else:
                if chk_state_neg.get() == False:
        ...
        def settrunk():
            if gettrkmode.get() == "Mode" or getiface1.get() == "" or getiface1.get() == "e.g. Eth0/0":
                    tkMessageBox.showinfo('错误', 'Please enter interface and choose switchport mode.', parent=window)
        ...
        def STPclick():
            if chk_state_neg.get() == False:
                if STPopt.get() == "Enable STP":
                    ...
                    if createv.get() == "" or createv.get() == "e.g. 1, 4094, 3-15":
                        tkMessageBox.showinfo('错误', 'Please enter a VLAN ID above.', parent=window)
                    ...
                    if createv.get() == "" or createv.get() == "e.g. 1, 4094, 3-15":
                        ...
                        tkMessageBox.showinfo('错误', 'Please enter required value(s).', parent=window)
                    else:
                        remote_conn.send("conf t\n")
                        sleep(0.2)
        ...
        def STPfeatclick():
            if STPfeatopt.get() == "Features":
                tkMessageBox.showinfo('错误', 'Please choose an option.', parent=window)
            else:
                if chk_state_neg.get() == False:
                    if STPfeatopt.get() == "STP portfast" or STPfeatopt.get() == "STP rootguard" or STPfeatopt.get() == "STP loopguard" or STPfeatopt.get() == "STP BPDUguard" or \
                       STPfeatopt.get() == "STP BPDUfilter":
                        if STPfeatinput.get() == "" or STPfeatinput.get() == "eg. e0/0":
                            tkMessageBox.showinfo('错误', 'Please enter an interface.', parent=window)
                        else:
        ...
        def showvlan():
            if showVLANopt.get() == "VLAN (id)":
                if shvlanid.get() == '' or shvlanid.get() == '<1-4094>':
                    tkMessageBox.showinfo('错误', 'Please enter a VLAN ID.', parent=window)
                else:
                    buff_size = 16384
                    sleep(0.5)
        ...
        def showportdtpvtp():
            if showportopt.get() == "SwitchPort":
                if shport.get() == '' or shport.get() == 'eg. fa0/0':
                    tkMessageBox.showinfo('错误', 'Please enter an interface (eg. fa0/0).', parent=window)
                else:
                    buff_size = 16384
                    sleep(0.5)
                ...
                if shport.get() == '' or shport.get() == 'status | counters | password':
                    tkMessageBox.showinfo('错误', 'Please enter an option (eg. status).', parent=window)
                else:
                    buff_size = 16384
                    sleep(0.5)
        ...
        def showSTP():
            if showSTPopt.get() == "STP interface":
                if shstp.get() == '' or shstp.get() == 'eg. fa0/0, vlan 1':
                    tkMessageBox.showinfo('错误', 'Please enter an interface or VLAN interface (eg. fa0/0, vlan 1).', parent=window)
                else:
                    buff_size = 16384
                    sleep(0.5)
                ...
                if shstp.get() == '' or shstp.get() == 'eg. Ethernet0/0, FastEthernet0/0':
                    tkMessageBox.showinfo('错误', 'Please enter an interface (eg. Ethernet0/0, FastEthernet0/0).', parent=window)
                else:
                    buff_size = 16384
                    sleep(0.5)
        ...
        def dhcpserverlease():
            if chk_state_neg.get() == False:
                if poolname.get() == "" or leasesubnet.get() == "" or leasesubnet.get() == "e.g. 192.168.1.0" or leasemask.get() == "" or\
                    ...
                    tkMessageBox.showinfo('错误', 'Minimal requirement is to enter a Pool Name, Lease Subnet and Lease Subnet Mask.', parent=window)
                else:
                    remote_conn.send("conf t \n")
                    sleep(0.2)
                ...
                defaultgateway.get() != "e.g. 192.168.1.254") and (leasetime.get() != "" and leasetime.get() != "e.g. 2 5 == 2 days 5 hrs"):
                    ...
                    tkMessageBox.showinfo('错误', 'To negate, please fill up required fields: Poolname + network/default-router/lease time/DNS.\
    ...
    def ACL():
        ...
        def setaclclick():
            if getiface.get() == '' or getiface.get() == 'eg. fa0/0, vty 0':
                tkMessageBox.showinfo('错误', 'Please enter a line or an interface.', parent=window)
            else:
            ...
            if chk_state_neg.get() == False:
                if PLRMvar == 1:
                    if listname.get() == "" or listname.get() == "eg. MyACL" or listseq.get() == "" or listseq.get() == "seq no.":
                        tkMessageBox.showinfo('错误', '错误 - please enter both the list name AND the sequence number.', parent=window)
                        ...
                        if ('Insertion failed' in output):
                            tkMessageBox.showinfo('错误', output, parent=window)
                        else:
                            tkMessageBox.showinfo('Prefix List', 'List created.', parent=window)
                    else:
                        tkMessageBox.showinfo('错误', 'Please enter required input values to create a Prefix List.', parent=window)
                else:
                    if listname.get() == "" or listname.get() == "eg. MyACL" or listseq.get() == "" or listseq.get() == "seq no.":
                        tkMessageBox.showinfo('错误', '错误 - please enter both the list name AND the sequence number.', parent=window)
                    elif rtemapval.get() == "":
                        ...
                        if rtemapopt.get() == "Match":
                            if (rtemapopt2.get() == "*weight" or rtemapopt2.get() == "*origin" or rtemapopt2.get() == "*dampening" or rtemapopt2.get() == "*vrf"):
                                tkMessageBox.showinfo('错误', 'These values can ONLY be Set, not Matched: weight, origin, dampening, vrf.', parent=window)
                ...
                if PLRMvar == 1:
                    if listname.get() == "" or listname.get() == "eg. MyACL":
                        tkMessageBox.showinfo('错误', '错误 - please enter both the list name.', parent=window)
                        ...
                        tkMessageBox.showinfo('错误', 'Please enter required input values to remove a Prefix List or a sequence within a Prefix List.', parent=window)
                else:
                    if listname.get() == "" or listname.get() == "eg. MyACL":
                        tkMessageBox.showinfo('错误', '错误 - please enter both the list name.', parent=window)
                        ...
                        tkMessageBox.showinfo('错误', 'Please enter required input values to remove a Route-Map, a sequence within a Route-map or \
        ...
        def setspeciallistconf():
            if speciallistnum.get() == "" or speciallistnum.get() == "ListNo." or speciallistval.get() == "" or \
               speciallistval.get() == "Regex for AS-ACL, <10:20> for comm list":
                tkMessageBox.showinfo('错误', '错误 - please enter both the list number AND the value.', parent=window)
            ...
            if BFDopt.get() == "Choose":
                ...
                if (getbfdiface.get() == "" or getbfdiface.get() == "eg. fa0/0") and (getbfdintv.get() == "" or getbfdintv.get() == "interval") and \
                   (getbfdrx.get() == "" or getbfdrx.get() == "min_rx") and (getbfdmult.get() == "" or getbfdmult.get() == "multiplier"):
                    tkMessageBox.showinfo('错误', 'Please enter all required information.', parent=window)
                else:
                    remote_conn.send("conf t \n")
                    sleep(0.3)
                ...
                if (getudldiface.get() == "" or getudldiface.get() == "eg. fa0/0"):
                    tkMessageBox.showinfo('错误', 'Please enter an interface at least.', parent=window)
                else:
                    remote_conn.send("conf t \n")
                    sleep(0.3)
                    remote_conn.send("int " + getbfdiface.get() + " \n")
...
#        blanklbl = Label(Mirrorframe, text="").grid(column=0, row=3)
        ...
        def spanport():
            if (getSessID.get() == "" or getSessID.get() == "<1-60>"):
                tkMessageBox.showinfo('错误', 'Please enter session ID. ', parent=window)
            else:
                ...
                if chk_state_neg == False:
                    if SPANvar.get() == 1:
                        if ("vlan" in getSessdstiface.get()) or ("VLAN" in getSessdstiface.get()):
                            tkMessageBox.showinfo('错误', 'Destination for SPAN can only be an interface, not a VLAN. ', parent=window)
                        else:
                        ...
                        if ("vlan" not in getSessdstiface.get()) or ("VLAN" not in getSessdstiface.get()):
                            tkMessageBox.showinfo('错误', 'Please enter a VLAN ID for the destination; remember to create it ("remote-span" VLAN type) in both the source\
AND destination device also. ', parent=window)
                        ...
                        if ("vlan" not in getSessiface.get()) or ("VLAN" not in getSessiface.get()):
                            tkMessageBox.showinfo('错误', 'Please enter a VLAN ID for the source; remember to create it ("remote-span" VLAN type) in both the source AND\
destination device also. ', parent=window)
                        elif ("vlan" in getSessdstiface.get()) or ("VLAN" in getSessdstiface.get()):
                            tkMessageBox.showinfo('错误', 'Destination for the remote RSPAN can only be an interface, not a VLAN. ', parent=window)
                        else:
                            if (getSessiface.get() != "" and getSessiface.get() != "eg. fa0/0, VLAN 2"):
                                remote_conn.send("conf t \n")
                                sleep(0.3)
    ...
    destination device also. ', parent=window)
        ...
        def slog():
            if chk_state_neg == False:
                if getSlog.get() == "":
                    tkMessageBox.showinfo('错误', 'Please enter the required information.', parent=window)
...
##        getenpasswd.grid(column=1, row=2)
        ...
        def adduserpass():
            ...
            if chk_state_neg.get() == False:
                if getusername.get() != "" and getpasswd.get() != "":
                    ...
                    tkMessageBox.showinfo('错误', 'Please enter both the username and password.', parent=window)
                ...
                if getusername.get() != "" and getpasswd.get() == "":
                    ...
                    tkMessageBox.showinfo('错误', 'Please enter either username only, or both username and password to negate.', parent=window)
                else:
                    remote_conn.send("conf t\n")
                    sleep(0.3)
                    remote_conn.send("no " + cmd)
        ...
        def addpriv():
            if chk_state_neg.get() == False:
                if getpriv.get() == "":
                    tkMessageBox.showinfo('错误', 'Please enter command to add to Read-Only user.', parent=window)
                else:
                    remote_conn.send("conf t\n")
                    sleep(0.3)
                ...
                if getpriv.get() == "":
                    tkMessageBox.showinfo('错误', 'Please enter command to remove from Read-Only user.', parent=window)
                else:
                    remote_conn.send("conf t\n")
                    sleep(0.3)
            ...
            if chk_state_neg.get() == False:
                if relodatetime.get() == "in":
                    if reloadmins.get() == "" or reloadmins.get() == "e.g. (in) 15, (at) 02:00 apr 21":
                        tkMessageBox.showinfo('错误', 'Please enter a number of seconds till/when the reload command initiates.', parent=window)
                    else:
        ...
        def banner():
            if bannerinput.get() == "":
                tkMessageBox.showinfo('错误', 'Please enter banner text.', parent=window)
            else:
                remote_conn.send("conf t\n")
                sleep(0.3)                    
    ...
    def HAvail():
        ...
        def LAG():
            ...
            if chk_state_neg.get() == False:
                if LAGgrp.get() == "" or (LAGports.get() == "" or LAGports.get() == "eg. +fa0/0+fa0/1.."):
                    tkMessageBox.showinfo('错误', 'Please enter both a bundle ID AND the ports/interfaces to be added to that bundle.', parent=window)
                else:
                    remote_conn.send("conf t\n")
                    sleep(0.3)                    
                ...
                if LAGgrp.get() == "":
                    tkMessageBox.showinfo('错误', 'Minimum information required is to enter a bundle ID (to be deleted).', parent=window)
                else:
                ...
                if chk_state_neg() == False:
                    if LAGotheropt.get() == "LACP system-priority":
                        ...
                        if LAGother2.get() == "" or LAGother2.get() == "eg. fa0/0":
                            tkMessageBox.showinfo('错误', 'Please enter a port(interface).', parent=window)
                        else:
                            remote_conn.send("conf t\n")
                            sleep(0.3)
        ...
        def RGR():
            ...
            if (RGRiface.get() == "" or RGRiface.get() == "eg. fa0/0") or (RGRID.get() == "" or RGRID.get() == "v1: <0-255>, v2: <0-4095>"):
                tkMessageBox.showinfo('错误', 'Please enter an interface and Protocol ID.', parent=window)
            else:
                if chk_state_neg.get() == False:
                    if (RGRvip.get() == "" or RGRvip.get() == "x.x.x.x"):
                        tkMessageBox.showinfo('错误', 'Please enter a VirtualIP to act as the gateway.', parent=window)
                    else:
                        remote_conn.send("conf t\n")
                        sleep(0.3)
            ...
            if RGRother.get() == "" or RGRother.get() == "value":
                ...
                if chk_state_neg.get() == False:
                    if RGRotheropt == "Preempt":
                        if (RGRiface.get() == "" or RGRiface.get() == "eg. fa0/0") or (RGRID.get() == "" or RGRID.get() == "v1: <0-255>, v2: <0-4095>"):
                            tkMessageBox.showinfo('错误', 'Please also enter the Protocol ID and the interface it is enabled on.', parent=window)
                        else:
                            remote_conn.send("conf t\n")
                            sleep(0.3)
                            remote_conn.send("int " + RGRiface.get() + "\n")
                        ...
                        if (RGRiface.get() == "" or RGRiface.get() == "eg. fa0/0"):
                            tkMessageBox.showinfo('错误', 'Please also enter the interface it is enabled on.', parent=window)
                        else:
                            remote_conn.send("conf t\n")
                            sleep(0.3)
                            remote_conn.send("int " + RGRiface.get() + "\n")
    ...
    def security():
            ...
            if chk_state_neg.get() == False:
                if PolicyID.get() == "" or PolicyID.get() == "eg. 1" or pskentry.get() == "" or pskentry.get() == "password" or PeerIDentry.get() == "" or \
                   PeerIDentry.get() == "eg. 0.0.0.0, MyPeerID":
                    ...
                    tkMessageBox.showinfo('错误', 'Please choose Encryption, Hash snd DH Group option.', parent=window)
                else:
                    remote_conn.send("conf t \n")
                    sleep(0.2)
                ...
                if PolicyID.get() == "" or PolicyID.get() == "eg. 1":
                    tkMessageBox.showinfo('错误', 'Please enter a PolicyID to delete it.', parent=window)
                else:
                    remote_conn.send("conf t \n")
                    sleep(0.2)
        ...
        def IPSECp2():
            if chk_state_neg.get() == False:
                if ipsecProfileName.get() == "" or ipsecProfileName.get():
                    tkMessageBox.showinfo('错误', 'Please enter an IPSEC Profile Name.', parent=window)
                elif ipsec_transform_set.get() == "Transform-Set" or ipsec_mode.get() == "Mode":
                    tkMessageBox.showinfo('错误', 'Please choose Transform-Set snd Mode.', parent=window)
                else:
                    remote_conn.send("conf t \n")
                    sleep(0.2)
                ...
                if ipsecProfileName.get() == "" or ipsecProfileName.get() == "eg. 1":
                    tkMessageBox.showinfo('错误', 'Please enter a Profile Name.', parent=window)
                else:
                    remote_conn.send("conf t \n")
                    sleep(0.2)
        ...
        def dmvpnclick():
            if dmvpntuniface.get() == "" or dmvpntuniface.get() == "eg. tun0":
                tkMessageBox.showinfo('错误', 'Please enter a tunnel interface to apply DMVPN configuration on.', parent=window)
            else:
                if chk_state_neg.get() == False:
                    res = ""
                    remote_conn.send("conf t \n")
        ...
        def createkchain():
            if keychainname.get() == "" or keychainname.get() == "keychain_name":
                tkMessageBox.showinfo('错误', 'Please enter keychain name.', parent=window)
            else:
                res = ""
                if chk_state_neg.get() == False:
                    remote_conn.send("conf t \n")
    ...
    def CLI():
        ...
        def CLIclick():
            if getCLIinput.get() == "":
                tkMessageBox.showinfo('错误', 'Please enter command.', parent=window)
            else:
                cmd = getCLIinput.get()
                buff_size = 16384
                sleep(0.5)
...
def defaultpage():
    ...
    Negateanyframe=LabelFrame(window,text=" 否定 ",font=('verdana', 8, 'bold'),padx=30,pady=7,width=100,height=100)
    ...
    bifacemain=LabelFrame(window,text=" 界面（下面的 “*应用” 命令必需） ",font=('verdana', 8, 'bold'),padx=122,pady=10,width=100,height=100)
    ...
    biface=LabelFrame(window,text=" 接口配置 ",font=('verdana', 8, 'bold'),padx=17,pady=0,width=100,height=100)
    biface.grid(row=1,column=0, sticky=("nsew"))


    chk_state_sec = BooleanVar()
    chk_state_sec.set(False)
    chk = Checkbutton(biface, text='次要IP', variable=chk_state_sec)
    chk.grid(column=2, row=2)

    chk_state_nosh = BooleanVar()
    chk_state_nosh.set(True)
    chk1 = Checkbutton(biface, text='启用', variable=chk_state_nosh)
    chk1.grid(column=3, row=2)




    lbl = Label(biface, text="设置IP和子网掩码:")
    lbl.grid(column=0, row=3)
    ipaddr = Entry(biface, bg='white', width=17, fg='grey')
    ipaddr.grid(column=1, row=3)
    ipaddr.insert(0, "IP。 输入“ del”删除")
    def handle_focus_in(_):
    ...
    def setinterfaceipclick():
        ...
        if getiface.get() == "" or getiface.get() == "e.g. fa0/0, fa0/0.2, vlan1, tun0, bvi1, po1 ..":
            tkMessageBox.showinfo('错误', '首先选择上方的接口.')
        else:
            if ((ipaddr.get() != "" and ipaddr.get().encode('utf-8') != "IP。 输入“ del”删除") and (ipmask.get() == "" or ipmask.get().encode('utf-8') == "子网掩码。 'del'删除"))\
               or ((ipaddr.get() == "" or ipaddr.get().encode('utf-8') == "IP。 输入“ del”删除") and (ipmask.get() != "" and ipmask.get().encode('utf-8') != "子网掩码。 'del'删除")):
                tkMessageBox.showinfo('错误', '请同时输入IP和子网掩码以配置IP。')
            elif chk_state_neg.get() == False and (getiface.get()[:2] == "tu") and ((tunsrc.get() != "" and tunsrc.get().encode('utf-8') != "源IP。 'del'删除") or\
                 (tundst.get() != "" and tundst.get().encode('utf-8') != "目的IP。 'del'删除")) and tunmode.get().encode('utf-8') == '模式':
                tkMessageBox.showinfo('错误', '添加隧道IP之前，请选择一种隧道模式。')
            elif chk_state_neg.get() == False and (ipaddr.get() == "del" or ipmask.get() == "del" or tunsrc.get() == "del" or tundst.get() == "del"):
                tkMessageBox.showinfo('错误', '选择“否定”以指定“ del”来删除IP地址。')
            else:
                if chk_state_neg.get() == False:
...
##                            tkMessageBox.showinfo('错误', 'The interface ' + getiface.get() + ' does not exist... Aborting. If BVI, ensure the Bridge Group has been created first.')
##                            return
##                        else:
##                            pass
...
##                        pass
                    ...
                    if 'Invalid input' in stripp:
                        sleep(0.2)
                        remote_conn.send("exit \n")
                        tkMessageBox.showinfo('错误', '接口 ' + getiface.get() + ' 不存在... 堕胎. 如果是BVI，请确保首先创建了桥组。')
                        return
                    else:
                        pass
                    
                    tuntxt = ""
                    if (getiface.get()[:2] == "tu") and ((tunsrc.get() != "" and tunsrc.get() != "源IP。 'del'删除") or\
                                                         (tundst.get() != "" and tundst.get() != "目的IP。 'del'删除")):
                        ...
                        sleep(0.3)
                        tuntxt = "配置了隧道IP-不要忘记添加路由以通过此隧道推送网络流量. "                           
                    else:
                        pass
...
##                        pass
                    ...
                    IPtxt = ""
                    if (ipaddr.get() != "" and ipaddr.get().encode('utf-8') != "源IP。 输入“ del”删除") and (ipmask.get() != "" and \
                        ipmask.get().encode('utf-8') != "子网掩码。 'del'删除"):
                    ...
                    sleep(0.2)
                    remote_conn.send("exit \n") 
                    tkMessageBox.showinfo('IP配置', IPtxt + Shutstatetxt + tuntxt + bridgetxt)


                else:
                    if (ipaddr.get() == "" or ipaddr.get().encode('utf-8') == "源IP。 输入“ del”删除") and (ipmask.get() == "" or ipmask.get().encode('utf-8') == "子网掩码。 'del'删除") \
                       and (tunsrc.get() == "" or tunsrc.get().encode('utf-8') == "源IP。 'del'删除") and (tundst.get() == "" or tundst.get().encode('utf-8') == "目的IP。 'del'删除"):
                        remote_conn.send("conf t \n")
                        sleep(0.3)
                        ...
                        remote_conn.send("exit \n")
                        sleep(0.2)
                        tkMessageBox.showinfo('IP配置', '接口被删除.')
                    else:
                        remote_conn.send("sh ip int " + getiface.get() + " \n")
                        sleep(0.5)
                        output = remote_conn.recv(2048).decode("utf-8")
                        ...
                        sleep(0.2)
                        if 'Invalid input' in stripp:
                            tkMessageBox.showinfo('错误', '接口 ' + getiface.get() + ' 不存在... 堕胎.')
                            return
                        else:
                            pass
                        ...
                        sleep(0.3)
                        tuntxt = ""
                        if (getiface.get()[:2] == "tu") and ((tunsrc.get() != "" and tunsrc.get().encode('utf-8') != "源IP。 'del'删除") or\
                                                             (tundst.get() != "" and tundst.get().encode('utf-8') != "目的IP。 'del'删除")):
                           
                            if tunsrc.get() == "del":
                                remote_conn.send("no tun so \n")
                                sleep(0.3)
                                tuntxt = tuntxt + "隧道源IP已删除. "
                            else:
                                pass
                            if tundst.get() == "del":
                                remote_conn.send("no tun dest \n")
                                sleep(0.3)
                                tuntxt = tuntxt + "隧道目标IP已删除. "
                            else:
                                pass
...
##                            pass
                        ...
                        if ipaddr.get() == "del" and ipmask.get() == "del":
                            if chk_state_sec.get() == False:
                                ...
                                IPtxt = "备用IP地址已删除. "
                        elif (ipaddr.get() != "" or ipaddr.get().encode('utf-8') != "源IP。 输入“ del”删除") and (ipmask.get() != "" or ipmask.get().encode('utf-8') != "子网掩码。 'del'删除"):
                            tkMessageBox.showinfo('错误', '请在IP和掩码字段中使用“ del”关键字来删除IP.')
                            sleep(0.2)   
                            remote_conn.send("exit \n")
                            sleep(0.2)   
                            remote_conn.send("exit \n")
...
##                            Shutstatetxt = "Interface is enabled. "
                        ...
                        sleep(0.2)
                        remote_conn.send("exit \n") 
                        tkMessageBox.showinfo('IP配置', IPtxt + tuntxt)
            
    btn = Button(biface, text="*应用", bg="orange", command=setinterfaceipclick)
    ...
    lbl = Label(biface, text="速度与双工:").grid(column=0, row=6)
    OPTIONS = [
    "auto",        
    "10",        
    "100",
    ...
    def setspeedduplex():
        if getiface.get() == "" or getiface.get() == "e.g. fa0/0, fa0/0.2, vlan1, tun0, bvi1, po1 ..":
            ...
            tkMessageBox.showinfo('错误', '只能为物理接口设置速度和双工.')
        elif ("f" in getiface.get() or "F" in getiface.get()) and spd.get() == "1000":
            tkMessageBox.showinfo('错误', 'FastEthernet的速度最多只能配置为100Mbps.')
        else:
            remote_conn.send("conf t \n")
            sleep(0.5)
            remote_conn.send("int " + getiface.get() + "\n")
            ...
            sleep(0.5)
            remote_conn.send("exit \n")
            res = "速度设置为 " + spd.get() + " 和双工 " + dup.get() + " 上 " + getiface.get()
            tkMessageBox.showinfo('速度与双工', res)
    btn = Button(biface, text="*应用", bg="orange", command=setspeedduplex)
    btn.grid(column=3, row=6)        

    blankdivider = Label(biface, text="").grid(column=0, row=8)
    
    lbl = Label(biface, text="其他设定:").grid(column=0, row=9)
            ...
            if chk_state_neg.get() == False:
                ...
                if otheroptn.get() == 'MAC address':
                    if otherinput.get() != "" and otherinput.get() != "xxxx.xxxx.xxxx":
                        ...
                        sleep(0.5)       
                        tkMessageBox.showinfo('其他界面设置', 'Interface MAC address configured.')
                    else:
                        tkMessageBox.showinfo('错误', '错误 - 请输入MAC地址以在接口上进行配置.')
                elif otheroptn.get() == 'dot1q':
                    if "." not in getiface.get():
                        tkMessageBox.showinfo('错误', '错误 - Dot1q封装只能应用于子接口.')
                    elif otherinput.get() == "":
                        tkMessageBox.showinfo('错误', '错误 - 请输入要在子接口上应用的VLAN ID.')
                    else:
                        sleep(0.5)
                        remote_conn.send("encap dot1Q " + otherinput.get() + "\n")
                        sleep(0.5)
                        tkMessageBox.showinfo('其他界面设置', '已配置Dot1q')
                    ...
                    output = remote_conn.recv(buff_size).decode("utf-8")
                    if otherinput.get() == "":
                        tkMessageBox.showinfo('错误', '错误 - 请输入VRF以在界面上申请.')
                    elif otherinput.get() not in output:
                        tkMessageBox.showinfo('错误', '错误 - 请先创建VRF（在“路由”页面中），然后再应用它.')
                    else:
                        sleep(0.5)
                        remote_conn.send("ip vrf forwarding " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('其他界面设置', '已配置VRF（如果已创建）.')
                    ...
                    if 'tu' not in getiface.get():
                        tkMessageBox.showinfo('错误', '错误 - 只能将fVRF应用于隧道接口.')
                    elif otherinput.get() == "":
                        tkMessageBox.showinfo('错误', '错误 - 请输入VRF以在界面上申请.')
                    elif otherinput.get() not in output:
                        tkMessageBox.showinfo('错误', '错误 - 请先创建VRF（在“路由”页面中），然后再应用它.')
                    ...
                    sleep(0.5)       
                    tkMessageBox.showinfo('其他界面设置', '配置为DHCP客户端。 删除了任何现有的IP地址.')
                elif otheroptn.get() == 'IP MTU(L3)':
                    if otherinput.get() != "" and otherinput.get() != "eg. 1400" and (68 <= int(otherinput.get()) <= 1500):
                        sleep(0.5)
                        remote_conn.send("ip mtu " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('其他界面设置', '配置的接口数据包MTU.')
                    else:
                        tkMessageBox.showinfo('错误', '错误 - 请输入一个MTU（68-1500）值以应用于该接口.')
                elif otheroptn.get() == 'MTU(L2)':
                    if otherinput.get() != "" and otherinput.get() != "eg. 1500" and (68 <= int(otherinput.get()) <= 1500):
                        sleep(0.5)
                        remote_conn.send("mtu " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('其他界面设置', '配置的接口框架MTU.')
                    else:
                        tkMessageBox.showinfo('错误', '错误 - 请输入一个MTU（1500-1530）值以应用于该接口.')                    
                elif otheroptn.get() == 'TCP MSS(L4)':
                    if otherinput.get() != "" and otherinput.get() != "eg. 1360" and (500 <= int(otherinput.get()) <= 1460):
                        ...
                        sleep(0.5)       
                        tkMessageBox.showinfo('其他界面设置', '已配置的MSS.')
                    else:
                        tkMessageBox.showinfo('错误', '错误 - 请输入一个MSS值（500-1460）以在界面上应用.')
                elif otheroptn.get() == 'VFR':
                    sleep(0.5)
                    remote_conn.send("ip virtual-reassembly \n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('其他界面设置', '为分段数据包启用了IP“ Virtual Reassembly”.')
                elif otheroptn.get() == 'Add to bridge':
                    if otherinput.get() == "<bridge-grp no.>" or otherinput.get() == "":
                        tkMessageBox.showinfo('错误', '错误 - 请输入一个“ Bridge Group”号码.')
                    else:
                        sleep(0.5)
                        remote_conn.send("int " + getiface.get() + " \n")
                        sleep(0.5)
                        remote_conn.send("bridge-group " + otherinput.get() + "\n")
                        tkMessageBox.showinfo('其他界面设置', getiface.get() + ' 已添加到桥组 ' + otherinput.get())
                elif otheroptn.get() == 'tun key':
                    if getiface.get()[:2] != 'tu': 
                        tkMessageBox.showinfo('错误', '错误 - 隧道密钥只能应用于隧道接口（例如tun0）.')
                    elif otherinput.get() == "" or otherinput.get() == "eg. 1":
                        tkMessageBox.showinfo('错误', '错误 - 请输入隧道密钥值以应用于接口.')
                    else:
                        remote_conn.send("tun key " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('其他界面设置', '已配置Tunnel key')                
                elif otheroptn.get() == 'keepalive':
                    if otherinput.get() == "" or otherinput.get() == "<secs> [<retries>]":
                        tkMessageBox.showinfo('错误', '错误 - 请输入一个keepalive值以应用到界面上.')
                    elif getiface.get()[:2] != 'tu':
                        findfirstdigits = re.search(r'\d+', otherinput.get()).group()
                        sleep(0.2)
                        remote_conn.send("keepalive " + findfirstdigits + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('其他界面设置', '接口保持活动状态')
                    else:
                        remote_conn.send("keepalive " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('其他界面设置', '隧道接口keepalive已配置.')
                elif otheroptn.get() == 'bandwidth':
                    if otherinput.get() == "" or otherinput.get() == "<kilobits>":
                        tkMessageBox.showinfo('错误', '错误 - 请输入带宽值以在接口上应用.')
                    else:
                        remote_conn.send("bandwidth " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('其他界面设置', '接口带宽已配置.')
                elif otheroptn.get() == 'IPSEC profile':
                    if getiface.get()[:2] != 'tu':
                        tkMessageBox.showinfo('错误', '错误 - 请将IPSEC配置文件应用于隧道接口（例如tun0).')
                    elif otherinput.get() == "" or otherinput.get() == "<IPSec_prof_name>":
                        tkMessageBox.showinfo('错误', '错误 - 请输入IPSEC配置文件以在界面上申请.')
                    else:                    
                        remote_conn.send("tunnel protection ipsec profile " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('其他界面设置', '隧道IPSEC配置文件已配置.')                    
                else:
                    remote_conn.send(otheroptn.get() + " " + otherinput.get() + "\n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('其他界面设置', 'ARP超时配置.')
            else:
                remote_conn.send("conf t \n")
                sleep(0.5)
                remote_conn.send("int " + getiface.get() + "\n")            
                if otheroptn.get() == 'MAC address':
                    ...
                    sleep(0.5)
                    tkMessageBox.showinfo('其他界面设置', '接口MAC地址重置为BIA.')
                elif otheroptn.get() == 'dot1q':
                    if "." not in getiface.get():
                        tkMessageBox.showinfo('错误', '错误 - 不是子接口.')
                    else:
                        sleep(0.5)
                        remote_conn.send("no encap dot1Q \n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('其他界面设置', 'Dot1q已删除.')
                elif otheroptn.get() == 'VRF':
                    sleep(0.5)
                    remote_conn.send("no ip vrf forwarding \n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('其他界面设置', 'VRF已删除.')
                elif otheroptn.get() == 'DHCP Client':
                    sleep(0.5)
                    remote_conn.send("no ip addr dhcp \n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('其他界面设置', '取消配置为DHCP客户端.')
                elif otheroptn.get() == 'IP MTU(L3)':
                    sleep(0.5)
                    remote_conn.send("no ip mtu \n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('其他界面设置', '接口数据包MTU重置为默认最大值（1500B).')
                elif otheroptn.get() == 'MTU(L2)':
                    sleep(0.5)
                    remote_conn.send("no mtu \n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('其他界面设置', '接口框架MTU重置为默认最大值（1500B).')                
                elif otheroptn.get() == 'TCP MSS(L4)':
                    sleep(0.5)
                    remote_conn.send("no ip tcp adjust-mss \n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('其他界面设置', '接口MSS重置为默认最大值（1460B).')
                elif otheroptn.get() == 'VFR':
                    sleep(0.5)
                    remote_conn.send("no ip virtual-reassembly \n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('其他界面设置', '禁用“ IP Virtual Reassembly”用于分段数据包.')
                elif otheroptn.get() == 'Add to bridge':
                    sleep(0.5)
                    remote_conn.send("no bridge-group " + otherinput.get() + "\n")
                    tkMessageBox.showinfo('其他界面设置', getiface.get() + ' 已从网桥组中删除 ' + otherinput.get())
                elif otheroptn.get() == 'tun key':
                    if getiface.get()[:2] != 'tu': 
                        tkMessageBox.showinfo('错误', '错误 - 不是隧道接口.')
                    else:
                        remote_conn.send("no tun key \n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('其他界面设置', '隧道密钥已删除.')                
                    ...
                    if getiface.get()[:2] != 'tu':
                        ...
                        sleep(0.5)       
                        tkMessageBox.showinfo('其他界面设置', '隧道保持连接已删除.')
                elif otheroptn.get() == 'bandwidth':
                        sleep(0.5)
                        remote_conn.send("no bandwidth \n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('其他界面设置', '接口带宽已删除.')
                elif otheroptn.get() == 'IPSEC profile':
                    if getiface.get()[:2] != 'tu':
                        tkMessageBox.showinfo('错误', '错误 - 请指定一个隧道接口（例如tun0).')
                    elif otherinput.get() == "":
                        tkMessageBox.showinfo('错误', '错误 - 请输入IPSEC配置文件以从界面中删除.')
                    else:                    
                        remote_conn.send("no tunnel protection ipsec profile " + otherinput.get() + "\n")
                        sleep(0.5)       
                        tkMessageBox.showinfo('其他界面设置', '隧道IPSEC配置文件已删除.') 
                else:
                    sleep(0.5)
                    remote_conn.send("no " + otheroptn.get() + " " + otherinput.get() + "\n")
                    sleep(0.5)       
                    tkMessageBox.showinfo('其他界面设置', 'ARP超时重置为默认值（4小时).')
            remote_conn.send("exit \n")
            sleep(0.5)
            remote_conn.send("exit \n")                
    btn = Button(biface, text="*应用", bg="orange", command=setotherconf)
    ...
    def helpotherconf():
        res = 'VFR (Virtual Fragmentation Reassembly) - 用于' +\
              ' 检测和防止一系列碎片攻击. \n\nDot1q - 用于中继的封装，“ ROUTER ON A STICK”等' +\
              ' 有效值: 1-4094. 仅在子接口上配置. 任何发往物理接口IP地址（如果已配置）' +\
              ' 的数据包都会被黑洞。.' +\
              '\n\nVRF - 像第3层VLAN一样工作. 必须先创建它（在“路由”选项卡中），然后才能应用它.\n\n' +\
              '次要IP - 您可以在接口上配置多个辅助IP. 如果未配置主IP，则辅助IP将 ' +\
              '保持不活动状态并隐藏，直到配置了主IP.' +\
              '\n\n桥接路由端口, \n 1. 创造 Bridge Group,\n 2. 创造 bvi界面\n 3. 其他设定 > "Add to bridge"'
        tkMessageBox.showinfo('其他界面设置帮助', res)
    btn = Button(biface, text="帮助", bg="yellow", command=helpotherconf)
    ...
    deviceconfframe=LabelFrame(window,text=" 设备配置和调整 ",font=('verdana', 8, 'bold'),padx=10,pady=8,width=100,height=100)
    deviceconfframe.grid(row=4,column=0, sticky=("nsew"))

    label = Label(deviceconfframe, text="创建静态ARP条目: ").grid(column=0, row=1)
    ...
    def setarp():
        ...
        if chk_state_neg.get() == False:
            ...
            remote_conn.send("exit \n")
            sleep(0.5)
            tkMessageBox.showinfo('ARP', '静态ARP已经配置.')
        else:
            remote_conn.send("no arp " + getarpip.get() + " " + getarpmac.get() + " arpa \n")
            sleep(0.5)
            remote_conn.send("exit \n")
            sleep(0.5)
            tkMessageBox.showinfo('ARP', '静态ARP已被删除.')          
    btn = Button(deviceconfframe, text="应用", font=('helvetica', 8), bg="orange", command=setarp)
    btn.grid(column=3, row=1, padx=13)

    label = Label(deviceconfframe, text="创建静态MAC条目: ").grid(column=0, row=2)
    ...
    def setmac():
        if getiface.get() == '' or getiface.get() == 'e.g. fa0/0, fa0/0.2, vlan1, tun0, bvi1, po1 ..':
            ...
            sleep(0.5)
            if chk_state_neg.get() == False:
                if getstvlan.get() == '' or getstvlan.get().encode('utf-8') == '可选VLAN':
                    remote_conn.send("mac-address-table static " + getstmac.get() + " interface " + getiface.get() + " \n")
                    sleep(0.5)
                    remote_conn.send("exit \n")
                    sleep(0.5)
                    tkMessageBox.showinfo('MAC', '配置静态MAC. 到达目的地的网络流量 ' + getstmac.get() + ' 将转发到 ' + getiface.get())
                else:
                    remote_conn.send("mac-address-table static " + getstmac.get() + " interface " + getiface.get() + " vlan " + getstvlan.get() + " \n")
                    sleep(0.5)
                    remote_conn.send("exit \n")
                    sleep(0.5)
                    tkMessageBox.showinfo('MAC', '配置静态MAC. VLAN中的网络流量 ' + getstvlan.get() +  ' 前往目的地 ' + getstmac.get() + \
                                          ' 将转发给 ' + getiface.get())
            else:
                if getstvlan.get() == '' or getstvlan.get().encode('utf-8') == '可选VLAN':
                    ...
                    remote_conn.send("exit \n")
                    sleep(0.5)
                    tkMessageBox.showinfo('MAC', '静态MAC已删除.')
                else:
                    ...
                    sleep(0.5)
                    tkMessageBox.showinfo('MAC', '静态MAC已删除.')        
    btn = Button(deviceconfframe, text="*应用", font=('helvetica', 8), bg="orange", command=setmac)
    btn.grid(column=3, row=2, padx=13, pady=2) 


    def option_changed_arpmacopt(*args):
        if arpmacopt.get().encode('utf-8') == "清除ARP（全部）" or arpmacopt.get().encode('utf-8') == "清除MAC（全部）":
            text = '无需输入'
        ...
        elif arpmacopt.get().encode('utf-8') == "清除ARP（接口）" or arpmacopt.get().encode('utf-8') == "清除MAC（接口）":
            text = 'eg. fa0/0'
            getmacarp.delete(0, END)
            getmacarp.config(fg='grey')
            getmacarp.insert(0, text)
            ...
            getmacarp.bind("<FocusOut>", handle_focus_out)
            getmacarp.bind("<FocusIn>", handle_focus_in)
        elif arpmacopt.get().encode('utf-8') == "清除ARP（IP）":
            text = 'eg. 1.2.3.4'
            getmacarp.delete(0, END)
            getmacarp.config(fg='grey')
            getmacarp.insert(0, text)
            ...
            getmacarp.bind("<FocusOut>", handle_focus_out)
            getmacarp.bind("<FocusIn>", handle_focus_in)
        elif arpmacopt.get().encode('utf-8') == "清除MAC（地址）":
            text = 'xxxx.xxxx.xxxx'
            getmacarp.delete(0, END)
            getmacarp.config(fg='grey')
            getmacarp.insert(0, text)
            ...
            getmacarp.bind("<FocusOut>", handle_focus_out)
            getmacarp.bind("<FocusIn>", handle_focus_in)
        elif arpmacopt.get().encode('utf-8') == "清除MAC（VLAN)":
            text = '<1-4094>'
            getmacarp.delete(0, END)
            getmacarp.config(fg='grey')
            getmacarp.insert(0, text)
    ...
    def arpmacclearset():
        if arpmacopt.get().encode('utf-8') == "清除ARP（全部）":
            remote_conn.send("clear arp \n")
            sleep(0.5)
            tkMessageBox.showinfo('ARP表', '清除所有的ARP表项.')
        elif arpmacopt.get().encode('utf-8') == "清除ARP（IP）":
            if getmacarp.get() != "" and ("." in getmacarp.get()):
                remote_conn.send("clear ip arp " + getmacarp.get() + "\n")
                sleep(0.5)
                tkMessageBox.showinfo('ARP表', 'ARP表项已经清除，且指定了IP属性.')
            else:
                tkMessageBox.showinfo('错误', '错误 - 请输入IP地址以清除关联的ARP条目.')
        elif arpmacopt.get().encode('utf-8') == "清除ARP（接口）":
            ...
            ("tun" in getmacarp.get()) or ("lo" in getmacarp.get())):
                ...
                sleep(0.5)
                tkMessageBox.showinfo('ARP表', '使用指定的接口属性清除了ARP表条目.')
            else:
                tkMessageBox.showinfo('错误', '错误 - 请输入一个界面（仅fa / gi / te / tun / lo / bvi）以清除关联的ARP条目.')
        elif arpmacopt.get().encode('utf-8') == "清除MAC（全部）":
                remote_conn.send("clear mac-address-table \n")
                sleep(0.5)
                tkMessageBox.showinfo('MAC Table', '所有MAC地址表项都已经清除.')
        elif arpmacopt.get().encode('utf-8') == "清除MAC（VLAN)":
            if getmacarp.get() != "" and getmacarp.get().isdigit() == True:
                remote_conn.send("clear mac-address-table vlan " + getmacarp.get() + " \n")
                sleep(0.5)
                tkMessageBox.showinfo('MAC表', '已清除所有MAC地址表条目的 ' + getmacarp.get() + '.')
            else:
                tkMessageBox.showinfo('错误', '错误 - 请输入VLAN ID清除关联的MAC条目.')
        elif arpmacopt.get().encode('utf-8') == "清除MAC（地址）":
            if getmacarp.get() != "" and ("." in getmacarp.get()):
                remote_conn.send("clear mac-address-table address " + getmacarp.get() + " \n")
                sleep(0.5)
                tkMessageBox.showinfo('MAC表', 'MAC地址表项已清除.')
            else:
                tkMessageBox.showinfo('错误', '错误 - 请输入一个MAC地址（XXXX.XXXX.XXXX）以清除关联的MAC地址条目.')
        elif arpmacopt.get().encode('utf-8') == "清除MAC（接口）":
            if getmacarp.get() != "":
                remote_conn.send("clear mac-address-table int " + getmacarp.get() + " \n")
                sleep(0.5)
                tkMessageBox.showinfo('MAC表', '接口的MAC地址表项已经清除 ' + getmacarp.get() + '.')
            else:
                tkMessageBox.showinfo('错误', '错误 - 请输入一个界面（仅fa / gi / te / tun / lo / bvi）以清除关联的MAC地址条目.')
        else:
            if chk_state_neg.get() == False:
                if getmacarp.get() != "" and getmacarp.get().isdigit() == True and (getmacarp.get() <= 1000000):
                    ...
                    tkMessageBox.showinfo('MAC表', 'MAC address table aging time has been configured.')
                else:
                    tkMessageBox.showinfo('错误', '错误 - 请输入有效的数字MAC地址老化时间.')
            else:
                remote_conn.send("no mac-address-table aging \n")
                sleep(0.5)
                tkMessageBox.showinfo('MAC表', 'MAC地址表老化时间已重置.')
    btn = Button(deviceconfframe, text="应用", font=('helvetica', 8), bg="orange", command=arpmacclearset)
        ...
        if chk_state_neg.get() == False:
            if TCPtuneopt.get() == "PMTUD":
                ...
                if getTCPtune.get() != "" and getTCPtune.get() != "[<2-20> (mins)]":
                    ...
                    sleep(0.3)
                    remote_conn.send("exit \n")
                    tkMessageBox.showinfo('TCP', '钟配置一次MTU重新校准的PMTUD每 ' + getTCPtune.get() + ' 分.')
                else:
                    remote_conn.send("ip tcp path-mtu-discovery \n")
                    sleep(0.3)
                    remote_conn.send("exit \n")
                    tkMessageBox.showinfo('TCP', 'PMTUD 已配置.')                 
            elif TCPtuneopt.get() == "TCP egress queue":
                if getTCPtune.get() != "" and getTCPtune.get() != "eg. 5 (segments)":
                    ...
                    sleep(0.3)
                    remote_conn.send("exit \n")
                    tkMessageBox.showinfo('TCP', 'TCP egress queue 已配置.')
                else:
                    tkMessageBox.showinfo('错误', '错误 - 请输入一个值.')                    
            elif TCPtuneopt.get() == "SYNWAIT-time":
                if getTCPtune.get() != "" and getTCPtune.get() != "eg. 30 (secs)":
                    ...
                    sleep(0.3)
                    remote_conn.send("exit \n")
                    tkMessageBox.showinfo('TCP', 'TCP SYNWAIT-time 已配置.')
                else:
                    tkMessageBox.showinfo('错误', '错误 - 请输入一个值.') 
            elif TCPtuneopt.get() == "TCP window-size":
                if getTCPtune.get() != "" and getTCPtune.get() != "eg. 65535": 
                    ...
                    sleep(0.3)
                    remote_conn.send("exit \n")
                    tkMessageBox.showinfo('TCP', 'TCP window-size 已配置.')
                else:
                    tkMessageBox.showinfo('错误', '错误 - 请输入一个值.') 
            elif TCPtuneopt.get().encode('utf-8') == "启用ECN":
                remote_conn.send("conf t \n")
                sleep(0.3)
                remote_conn.send("ip tcp ecn \n")
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', '已配置TCP Explicit Congestion Notification。 注意：远程对等方也必须启用ECN才能使用此功能.')
            elif TCPtuneopt.get().encode('utf-8') == "启用Selective-ACK":
                remote_conn.send("conf t \n")
                sleep(0.3)
                remote_conn.send("ip tcp selective-ack \n")
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP Selective-ACK(SACK) 已配置.')
            else:
                remote_conn.send("conf t \n")
                sleep(0.3)
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP timestamp 已配置.')
        else:
            if TCPtuneopt.get() == "PMTUD":
                remote_conn.send("conf t \n")
                sleep(0.3)
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP PMTUD 已禁用.') 
            elif TCPtuneopt.get() == "TCP egress queue":
                remote_conn.send("conf t \n")
                sleep(0.3)
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP egress-queue-max 重置为默认值.') 
            elif TCPtuneopt.get() == "SYNWAIT-time":
                remote_conn.send("conf t \n")
                sleep(0.3)
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP SYNWAIT-time 重置为默认值.') 
            elif TCPtuneopt.get() == "TCP window-size":
                remote_conn.send("conf t \n")
                sleep(0.3)
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP window-size 重置为默认值.') 
            elif TCPtuneopt.get().encode('utf-8') == "启用ECN":
                remote_conn.send("conf t \n")
                sleep(0.3)
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP Explicit Congestion Notification 已禁用.') 
            elif TCPtuneopt.get().encode('utf-8') == "启用Selective-ACK":
                remote_conn.send("conf t \n")
                sleep(0.3)
                ...
                sleep(0.3)
                remote_conn.send("exit \n")
                tkMessageBox.showinfo('TCP', 'TCP Selective-ACK 已禁用.')
            else:
                remote_conn.send("conf t \n")
                sleep(0.3)
                remote_conn.send("ip tcp timestamp \n")
                sleep(0.3)
                remote_conn.send("exit \n")                
            tkMessageBox.showinfo('TCP', 'TCP timestamp 已禁用.')
    btn = Button(deviceconfframe, text="应用", font=('helvetica', 8), bg="orange", command=TCPtune)
    ...
    chk = Checkbutton(deviceconfframe, text="MAC学习", font=("TkDefaultFont", 8), variable=chk_MAClearn)
    chk.grid(column=1, row=6, sticky='e', columnspan=2, pady=(15,0))
    


    def option_changed_bridgefilter(*args):
        if bridgefilter.get().encode('utf-8') == "可选的":
            text = ''
    ...
    def createbridgeG():
        if createbridge.get() == "" or createbridge.get() == "eg. 5":
            tkMessageBox.showinfo('错误', '请输入一个BridgeGroup号码.')
        else:
            if chk_state_neg.get() == False: 
                remote_conn.send("conf t \n")
                sleep(0.3)
                ...
                remote_conn.send("exit \n")
                sleep(0.3)
                tkMessageBox.showinfo('网桥配置', '桥已创建. ' + res)
            else:
                remote_conn.send("conf t \n")
                sleep(0.3)
    ...
    btn = Button(deviceconfframe, text="应用", font=('helvetica', 8), bg="orange", command=createbridgeG)
    btn.grid(row=6, column=3, pady=(15,0)) 

    def bridgeGOpt():
        if createbridge.get() == "" or createbridge.get() == "eg. 5":
            tkMessageBox.showinfo('错误', '请输入“ BridgeGroup”编号')
        elif bridgefilter.get().encode('utf-8') == "可选的":
            tkMessageBox.showinfo('错误', '请选择一个选项.')
        else:
            if chk_state_neg.get() == False:
                remote_conn.send("conf t \n")
                sleep(0.3)
    ...
    def showintconf():
        if showintopt.get().encode('utf-8') == "接口统计":
            if getiface.get() == '' or getiface.get() == 'e.g. fa0/0, fa0/0.2, vlan1, tun0, bvi1, po1 ..':
                tkMessageBox.showinfo('错误', '首先选择上方的接口.')
            else:
                buff_size = 16384
                ...
                while not remote_conn.recv_ready():
                    ...
                    sleep(0.5)
                output = remote_conn.recv(buff_size).decode("utf-8")
                tkMessageBox.showinfo('当前接口配置', output)
        elif showintopt.get().encode('utf-8') == "接口配置":
            if getiface.get() == '' or getiface.get() == 'e.g. fa0/0, fa0/0.2, vlan1, tun0, bvi1, po1 ..':
                tkMessageBox.showinfo('错误', '首先选择上方的接口.')
            else:
                rep = getiface.get()
                ...
                output = remote_conn.recv(buff_size).decode("utf-8")
                tkMessageBox.showinfo('当前接口配置', output)
        elif showintopt.get().encode('utf-8') == "L3接口状态":
            if getiface.get() == '' or getiface.get() == 'e.g. fa0/0, fa0/0.2, vlan1, tun0, bvi1, po1 ..':
                tkMessageBox.showinfo('错误', '首先选择上方的接口.')
            else:
                buff_size = 16384
            ...
            output = remote_conn.recv(buff_size).decode("utf-8")
            tkMessageBox.showinfo('当前接口配置', output)        
    btn = Button(showconfframe, text="显示", bg="orange", command=showintconf)
    btn.place(x=300, y=0)
    ...
    def showintconf2():
        if showintopt2.get().encode('utf-8') == "MAC表":
            buff_size = 16384
            sleep(0.5)
            ...
            output = remote_conn.recv(buff_size).decode("utf-8")
            tkMessageBox.showinfo('当前接口配置', output)
        elif showintopt2.get().encode('utf-8') == "MAC表（总计）":
            buff_size = 16384
            sleep(0.5)
            ...
            output = remote_conn.recv(buff_size).decode("utf-8")
            tkMessageBox.showinfo('当前接口配置', output)
        elif showintopt2.get().encode('utf-8') == "ARP表":
            buff_size = 16384
            sleep(0.5)
            ...
            if showintopt2.get().encode('utf-8') == "硬件状态":
                cmd = "show env \n"
            elif showintopt2.get().encode('utf-8') == "硬件状态（详细信息）":
                cmd = "show env all \n"                
            elif showintopt2.get().encode('utf-8') == "硬体资讯":
                cmd = "show inv \n"
            elif showintopt2.get().encode('utf-8') == "BUFFER状态":
                cmd = "show buff \n"
            elif showintopt2.get().encode('utf-8') == "时钟":
                cmd = "show clock \n"
            elif showintopt2.get().encode('utf-8') == "正常运行时间":
                cmd = "show ver | i uptime | returned | restarted \n"
            elif showintopt2.get().encode('utf-8') == "版":
                cmd = "show ver | s IOS \n"
            elif showintopt2.get() == "CPU":
                cmd = "show proc cpu hist \n"
            elif showintopt2.get().encode('utf-8') == "记忆":
                cmd = "show proc mem \n"
            elif showintopt2.get().encode('utf-8') == "双工/速度（已配置）":
                cmd = "show run | i interface | duplex | speed \n"
            elif showintopt2.get().encode('utf-8') == "双工/速度（活动）":
                cmd = "show int | i dup| up \n"                
            elif showintopt2.get().encode('utf-8') == "TCP连接":
                cmd = "show tcp brief nume \n"
            elif showintopt2.get().encode('utf-8') == "TCP统计":
                cmd = "show tcp stat \n"
            elif showintopt2.get().encode('utf-8') == "监听端口":
            ...
            output = remote_conn.recv(buff_size).decode("utf-8")
            tkMessageBox.showinfo('当前接口配置', output)
    btn = Button(showconfframe, text="显示", bg="orange", command=showintconf2)
...
if __name__ == "__main__":
        ...
        main()
        window.withdraw()
        jumphostip = askstring("", "输入跳转主机IP（如果没有，请输入“ n”）:")
        ip = askstring("", "输入主机IP:")
        if jumphostip == "" or ip == "":
            window.destroy()
